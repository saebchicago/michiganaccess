...
import {
  Search, Shield, Heart, ArrowRight, ArrowDown, ExternalLink,
} from "lucide-react";
...
function AnimStat({ value, label, source, className = "" }: { value: number; label: string; source: string; className?: string }) {
  const { value: display, ref } = useCountUp<HTMLParagraphElement>(value, 1500);
  return (
    <div className={`text-center ${className}`}>
      <p ref={ref} className="text-4xl lg:text-5xl font-bold tabular-nums">{display.toLocaleString()}</p>
  ...
  return (
    <Layout>
      ...
      <section className="min-h-screen flex items-center justify-center bg-michigan-blue text-white px-4 relative">
        <div className="max-w-3xl text-center">
          ...
          <motion.div {...fadeIn} transition={{ duration: 0.6, delay: 0.3 }} className="grid grid-cols-1 sm:grid-cols-3 gap-8 mt-12">
            ...
            <AnimStat value={1544250} label="food insecure residents" source="Feeding America 2024" />
            {/* Source: United For ALICE 2023 */}
            <div className="text-center">
              <p className="text-4xl lg:text-5xl font-bold tabular-nums">41%</p>
              <p className="text-sm mt-1 opacity-80">of households below ALICE threshold</p>
              <p className="text-[9px] opacity-50 mt-0.5">United For ALICE 2023</p>
            </div>
            {/* Source: HMA March 2025 */}
            <AnimStat value={1772000} label="on Medicaid managed care" source="HMA March 2025" />
          </motion.div>
        </div>
        ...
        <div className="container max-w-3xl">
          <motion.div {...fadeIn}>
            <p className="text-lg lg:text-xl text-muted-foreground leading-relaxed mb-8">
              Of <strong className="text-foreground">1.77 million</strong> Medicaid beneficiaries,
              only <strong className="text-foreground">27%</strong> are screened for social needs.
              Of those screened, just <strong className="text-red-600">11%</strong> get connected to help.
            </p>
          </motion.div>
          ...
          <motion.p {...fadeIn} transition={{ duration: 0.6, delay: 0.4 }}
            ...
            89% of identified needs never reach resolution. That's not a knowledge problem. It's an infrastructure problem.
          </motion.p>
        </div>
      </section>
        ...
        <div className="container max-w-4xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              { num: "47th", label: "Michigan's national ranking in psychiatric bed availability", source: "Treatment Advocacy Center" },
              /* Source: MDHHS */
              ...
              <motion.div key={s.num} {...stagger(i)}>
                <Card><CardContent className="py-6 text-center">
                  <p className="text-3xl lg:text-4xl font-bold text-foreground">{s.num}</p>
                  <p className="text-sm text-muted-foreground mt-2">{s.label}</p>
                  <p className="text-[9px] text-muted-foreground/60 mt-1">Source: {s.source}</p>
                </CardContent></Card>
              </motion.div>
            ))}
          </div>
          <motion.div {...fadeIn} transition={{ delay: 0.6 }} className="mt-8 text-center">
            ...
            <Link to="/behavioral-health" className="text-sm text-primary hover:underline mt-3 inline-flex items-center gap-1">
              Explore the full Behavioral Health Dashboard <ArrowRight className="h-3.5 w-3.5" />
            </Link>
          </motion.div>
        </div>
      </section>
            ...
            <Card className="mt-8 border-teal-200/50 dark:border-teal-900/30 bg-teal-50/30 dark:bg-teal-950/10">
              <CardContent className="py-5">
                <p className="text-sm text-foreground leading-relaxed">
                  <strong>Sturgis Hospital</strong> converted to Michigan's first Rural Emergency Hospital in July 2023 — trading inpatient beds for guaranteed $291K/month federal payments and 105% outpatient rates.
                </p>
                <p className="text-[9px] text-muted-foreground/60 mt-1">Source: Sturgis Hospital / CMS REH Program</p>
              </CardContent>
            </Card>
          </motion.div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.div {...stagger(0)}>
              <Card className="h-full border-red-200/50 dark:border-red-900/30">
                <CardContent className="py-5">
                  <p className="text-xs font-bold text-red-600 uppercase tracking-wider mb-2">The Crisis</p>
                  <p className="text-sm text-foreground leading-relaxed">
                    In Flint, 70% of residents didn't trust government assurances about water safety after the 2014 crisis. Lead testing rates failed to increase despite known exposure.
                  </p>
                  <p className="text-[9px] text-muted-foreground/60 mt-2">Source: Wayne State / American Scientist</p>
                </CardContent>
              </Card>
            </motion.div>
            <motion.div {...stagger(1)}>
              <Card className="h-full border-teal-200/50 dark:border-teal-900/30">
                <CardContent className="py-5">
                  <p className="text-xs font-bold text-teal-600 uppercase tracking-wider mb-2">The Response</p>
                  <p className="text-sm text-foreground leading-relaxed">
                    Rx Kids — Flint's universal perinatal cash transfer — achieved 98% enrollment and raised trust in healthcare institutions by 10 percentage points.
                  </p>
                  <p className="text-[9px] text-muted-foreground/60 mt-2">Source: rxkids.org / U-M Poverty Solutions</p>
                </CardContent>
              </Card>
            </motion.div>
          </div>
      ...
      <section className="bg-gradient-to-br from-michigan-blue to-michigan-teal text-white px-4 py-20">
        <div className="container max-w-4xl text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
            {[
              { icon: Search, title: "Connects", desc: "15,000+ resources across all 83 counties, searchable by ZIP" },
              { icon: Shield, title: "Verifies", desc: "Every data point traced to a public source. 26+ verified anchors." },
              { icon: Heart, title: "Navigates", desc: "3-question triage routes residents to help in 60 seconds" },
            ].map((item, i) => (
              <motion.div key={item.title} {...stagger(i)} className="text-center">
      ...
      <section className="py-20 px-4 bg-background">
        <div className="container max-w-4xl">
          <motion.h2 {...fadeIn} className="text-2xl font-bold text-foreground text-center mb-8">Explore the Data</motion.h2>
