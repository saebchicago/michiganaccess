...
import { Zap, Home, Flame, Phone, ExternalLink, MapPin, DollarSign, Snowflake, Shield, ChevronDown, ChevronUp } from "lucide-react";
...
const PROGRAMS: EnergyProgram[] = [
  {
    ...
    provider: "MDHHS",
    description: "Federal program helping low-income households pay heating and cooling costs. Provides direct vendor payments to utility companies.",
    maxFPL: 150,
    types: ["electric", "gas"],
    phone: "844-799-9876",
  ...
  },
  {
    ...
    provider: "MDHHS",
    description: "Emergency assistance to prevent utility shutoff, restore service, or help with energy-related emergencies.",
    maxFPL: 150,
  ...
  },
  {
    ...
    description: "Free home energy audits and upgrades: insulation, air sealing, furnace repair/replacement, and energy-efficient windows.",
    maxFPL: 200,
  ...
  },
  {
    ...
    provider: "DTE Energy",
    description: "Discounted monthly bills based on income level. Includes arrears forgiveness and payment plans.",
    maxFPL: 150,
    types: ["electric", "gas"],
    phone: "800-477-4747",
    ...
    coverageArea: ["Wayne", "Oakland", "Macomb", "Monroe", "Washtenaw", "Livingston", "Lenawee", "St. Clair", "Lapeer"],
  ...
  },
  {
    ...
    description: "Payment plans and protection from shutoff for income-qualifying households. Includes winter protection programs.",
    maxFPL: 150,
    types: ["electric", "gas", "crisis"],
    phone: "800-477-5050",
    url: "https://www.consumersenergy.com/residential/billing-and-payment/help-with-your-bill",
    coverageArea: ["Kent", "Ottawa", "Kalamazoo", "Jackson", "Calhoun", "Eaton", "Ingham", "Bay", "Saginaw", "Midland", "Genesee", "Isabella", "Allegan", "Berrien", "Muskegon"],
  ...
  },
  {
    ...
    description: "State law prohibits utility shutoffs Nov 1 – Mar 31 for eligible households. Must enroll through your utility.",
    maxFPL: 150,
    types: ["electric", "gas", "crisis"],
  ...
  },
  {
    ...
    description: "Nonprofit emergency energy assistance. Provides grants to prevent shutoff and restore service for Michigan families.",
    maxFPL: 200,
    types: ["electric", "gas", "crisis"],
    phone: "800-866-8429",
  ...
  },
  {
    ...
    description: "Federal Inflation Reduction Act rebates for heat pumps, insulation, electrical panels, and energy-efficient appliances.",
    maxFPL: 300,
    types: ["weatherization"],
...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  electric: { label: "Electric", color: "bg-michigan-gold/15 text-michigan-gold border-michigan-gold/25" },
  gas: { label: "Gas/Heating", color: "bg-michigan-coral/15 text-michigan-coral border-michigan-coral/25" },
  weatherization: { label: "Weatherization", color: "bg-michigan-teal/15 text-michigan-teal border-michigan-teal/25" },
  crisis: { label: "Crisis/Emergency", color: "bg-destructive/15 text-destructive border-destructive/25" },
};
  ...
  const matchedPrograms = useMemo(() => {
    return PROGRAMS.filter(p => {
      ...
      if (county && p.coverageArea !== "statewide") {
        if (Array.isArray(p.coverageArea) && !p.coverageArea.includes(county)) return false;
      }
      // Type check
      if (typeFilter !== "all" && !p.types.includes(typeFilter as any)) return false;
      return true;
    });
    ...
    <Card>
      <CardHeader className={compact ? "pb-2" : ""}>
        ...
        <p className="text-xs text-muted-foreground">
          Find LIHEAP, weatherization, and utility discount programs you may qualify for.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div>
            <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">County</label>
            <Select value={county} onValueChange={setCounty}>
              <SelectTrigger className="h-9 text-xs"><MapPin className="mr-1 h-3 w-3" /><SelectValue placeholder="All Counties" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            <label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider mb-1 block">
              ...
              {eligibility.fplPercent && <span className="text-michigan-teal ml-1">(from profile)</span>}
            </label>
            ...
            {eligibility.fplPercent && (
              <p className="text-[10px] text-muted-foreground">Using income from your eligibility profile</p>
            )}
          </div>
          <div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            <strong className="text-foreground">{matchedPrograms.length}</strong> programs you may qualify for
          </p>
          {effectiveFPL <= 150 && (
        ...
        <div className="space-y-2">
          {matchedPrograms.map((p) => (
            <Collapsible key={p.id} open={expanded === p.id} onOpenChange={(open) => setExpanded(open ? p.id : null)}>
              <div className="rounded-lg border border-border bg-card hover:bg-muted/30 transition-colors">
                <CollapsibleTrigger className="w-full p-3 flex items-start gap-3 text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-semibold text-foreground leading-tight">{p.name}</h4>
                    <p className="text-[11px] text-muted-foreground mt-0.5">{p.provider} · Up to {p.maxFPL}% FPL</p>
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {p.types.map(t => (
                        <Badge key={t} variant="outline" className={`text-[9px] ${TYPE_LABELS[t].color}`}>
                          {TYPE_LABELS[t].label}
                        </Badge>
                      ))}
                      {p.coverageArea === "statewide" ? (
                        <Badge variant="secondary" className="text-[9px]">Statewide</Badge>
                      ) : (
                        <Badge variant="secondary" className="text-[9px]">{(p.coverageArea as string[]).length} counties</Badge>
                      )}
                    </div>
                  </div>
          ...
          {matchedPrograms.length === 0 && (
            <div className="text-center py-6 text-muted-foreground">
              ...
              <p className="text-xs mt-1">Try adjusting your income level or county filter.</p>
            </div>
          )}
        </div>
