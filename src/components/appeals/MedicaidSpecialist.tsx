...
const LEGAL_AID_BY_REGION: Record<string, { name: string; phone: string; url: string }[]> = {
  "Southeast": [
    { name: "Michigan Legal Services (Detroit)", phone: "313-964-4130", url: "https://michiganlegalservices.org" },
    { name: "Lakeshore Legal Aid", phone: "888-783-8190", url: "https://lakeshorelegalaid.org" },
  ],
  "West": [
    { name: "Legal Aid of Western Michigan", phone: "616-774-0672", url: "https://lawestmi.org" },
  ],
  "North/UP": [
    { name: "Legal Services of Northern Michigan", phone: "231-947-0122", url: "https://lsnm.org" },
  ],
  "Statewide": [
    { name: "Michigan Legal Help", phone: "N/A", url: "https://michiganlegalhelp.org" },
    { name: "State Long-Term Care Ombudsman", phone: "866-485-9393", url: "https://www.michigan.gov/osa" },
  ],
};
  ...
  return (
    <section className="space-y-6" aria-labelledby="medicaid-heading">
      <div className="text-center">
        <h2 id="medicaid-heading" className="text-2xl font-bold text-foreground">
          Medicaid & MIChild Appeal Specialist
        </h2>
        <p className="mt-2 text-muted-foreground max-w-2xl mx-auto">
          Medicaid fair hearings have a{" "}
          <span className="font-semibold text-accent">95% success rate</span>{" "}
          when properly filed. Use these templates and connect with free legal assistance.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="text-center p-4">
          ...
          <p className="text-sm text-muted-foreground">Fair hearing success rate</p>
        </Card>
        <Card className="text-center p-4">
          <Phone className="h-6 w-6 mx-auto text-accent mb-2" />
          <p className="text-sm font-semibold">Request a Fair Hearing</p>
          <a href="tel:5173737500" className="text-primary font-bold text-lg hover:underline">517-373-7500</a>
          <p className="text-xs text-muted-foreground mt-1">MDHHS Administrative Hearings</p>
        </Card>
        <Card className="text-center p-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">
            ...
            {county && (
              <Badge variant="outline" className="ml-2 text-xs">{county} County</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2">
            {Object.entries(LEGAL_AID_BY_REGION).map(([region, orgs]) => (
              <div key={region}>
                <h4 className="text-sm font-semibold text-muted-foreground mb-2">{region} Michigan</h4>
                {orgs.map((org) => (
                  <div key={org.name} className="mb-3 text-sm">
                    ...
                    {org.phone !== "N/A" && (
                      <a href={`tel:${org.phone.replace(/\D/g, "")}`} className="text-muted-foreground hover:text-foreground">
                        {org.phone}
                      </a>
                    )}
                  </div>
