...
const STEPS = [
  ...
  { title: "Equity Atlas", desc: "D3 choropleth map with 10 data layers across all 83 counties." },
];
...
export default function GuidedTour() {
  ...
  const handleNext = () => {
    ...
    else { setActive(false); sessionStorage.setItem("tour-done", "1"); }
  };

  const handleSkip = () => { setActive(false); sessionStorage.setItem("tour-done", "1"); };
      ...
      >
        ...
        >
          ...
          <div className="flex justify-center gap-1 mt-3">
            {STEPS.map((_, i) => (
              <span key={i} className={`h-1 rounded-full transition-all ${i === step ? "w-4 bg-primary" : "w-1 bg-muted-foreground/30"}`} />
            ))}
          </div>
        </motion.div>
      </motion.div>
