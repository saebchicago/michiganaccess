  ...
  return (
    <div className={`${bg} print:hidden`}>
      <div className="container flex items-center gap-2 py-1.5 text-sm">
        ...
        <button onClick={() => setDismissed(true)} className="shrink-0 p-0.5" aria-label="Dismiss">
            ...
            <div className="container pb-2 space-y-1">
              {top.map((a) => (
                <div key={a.id} className="text-[11px] opacity-90">
                  <strong>{a.event}</strong> — {a.areaDesc?.split(";").slice(0, 3).join(", ")}
                  {a.expires && <span className="opacity-70"> · Until {new Date(a.expires).toLocaleString("en-US", { weekday: "short", hour: "numeric", minute: "2-digit" })}</span>}
                </div>
              ))}
              <p className="text-[9px] opacity-60">Source: National Weather Service</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
