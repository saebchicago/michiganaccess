...
import { DollarSign, ArrowRight, Share2, Info } from "lucide-react";
...
export default function TaxComparisonCalculator() {
  ...
  const taxA = useMemo(() => calculateTaxBurden({ annualSalary: salary, filingStatus: status, dependents: deps, city: cityA, isResident: true }), [salary, status, deps, cityA]);
  const taxB = useMemo(() => calculateTaxBurden({ annualSalary: salary, filingStatus: status, dependents: deps, city: cityB, isResident: true }), [salary, status, deps, cityB]);
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              ...
              <Input type="number" value={salary} onChange={(e) => setSalary(parseInt(e.target.value) || 0)} className="font-mono" />
            </div>
            <div>
        ...
        <Card>
          <CardContent className="p-5">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  <tr className="font-semibold">
                    ...
                    <td className="py-2 text-xs text-right">{fmt(taxA.totalTax)}</td>
                    <td className="py-2 text-xs text-right">{fmt(taxB.totalTax)}</td>
                    <td className="py-2 text-xs text-right"><DiffCell a={taxA.totalTax} b={taxB.totalTax} /></td>
                  </tr>
                  <tr>
                  ...
                  <tr className="font-bold text-primary">
                    ...
                    <td className="py-2 text-xs text-right">{fmt(taxA.takeHomePay)}</td>
                    <td className="py-2 text-xs text-right">{fmt(taxB.takeHomePay)}</td>
                    <td className="py-2 text-xs text-right"><DiffCell a={-taxA.takeHomePay} b={-taxB.takeHomePay} /></td>
                  </tr>
                  <tr>
            ...
            <div className="flex items-start gap-2 mt-3 text-[9px] text-muted-foreground">
              ...
              <span>Estimated for educational purposes. Uses representative rates. Property tax based on median home value. Auto insurance from 2025 rate filings. Consult a tax professional. Sources: MI Treasury, IRS, city ordinances, Quadrant Info Services.</span>
            </div>
          </CardContent>
        </Card>
      </motion.div>
