...
function generateRecommendations(dataMap: Record<string, number>): string[] {
  ...
  const gaps = Object.entries(dataMap)
    ...
    .map(([m, v]) => ({ measure: m, gap: v - (MI_STATE_AVERAGES[m] || 0), value: v }))
    .filter((g) => g.gap > 1)
...
export default function CommunityReportCard({ zipCode, data }: Props) {
  const handleGenerate = () => {
    ...
    data.forEach((d) => { dataMap[d.short_question_text] = d.data_value; });
    ...
    const KEY_MEASURES = ["Diabetes", "Obesity", "Depression", "Lack of Health Insurance", "Current Smoking", "No Leisure-Time Physical Activity"];
    const stats = KEY_MEASURES.map((m) => ({
      name: m,
      value: dataMap[m],
      stateAvg: MI_STATE_AVERAGES[m],
      above: dataMap[m] && MI_STATE_AVERAGES[m] ? dataMap[m] > MI_STATE_AVERAGES[m] + 1 : false,
