...
const PROGRAMS: Program[] = [
  ...
  { name: "GSRP (Great Start Readiness)", description: "Free preschool for 4-year-olds.", eligibility: "250", link: "/resources", category: "family", situations: ["other"], urgency: 3 },
  { name: "Michigan Saves Green Loans", description: "Low-interest financing for solar, HVAC, and insulation.", eligibility: "all", link: "/environment", category: "energy", situations: ["energy"], urgency: 2 },
  { name: "MIHI Broadband Subsidy", description: "Affordable high-speed internet.", eligibility: "200", link: "/environment", category: "energy", situations: ["bills", "other"], urgency: 2 },
];
...
export default function DiscoveryWizard({ open, onOpenChange }: DiscoveryWizardProps) {
  ...
  const results = useMemo(() => {
    ...
    return PROGRAMS
      .filter((p) => {
        if (!p.situations.includes(situation) && situation !== "other") return false;
                ...
                {SITUATIONS.map((s) => (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-lg border p-3 text-left text-sm transition-all ${
                      situation === s.id ? `${s.color} border-2 ring-1 ring-primary/30` : "border-border hover:border-primary/40"
                    }`}
                  >
            ...
            <motion.div key="s2" {...step_anim} className="space-y-5 py-2">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <Badge variant="outline" className="text-[10px]">{fplPct}% FPL</Badge>
                </div>
              </div>
