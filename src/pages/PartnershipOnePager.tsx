    ...
    <Layout>
      ...
      <div className="container py-8 max-w-3xl one-pager">
        ...
        <div className="flex items-center justify-between mb-8 no-print">
          ...
          <Button onClick={handlePrint} className="bg-michigan-teal hover:bg-michigan-teal/90 text-white">
        ...
        <div className="space-y-8">
          ...
          {/* What We Do */}
          <section>
            <h2 className="text-lg font-bold text-foreground mb-3 border-l-4 border-michigan-teal pl-3">What We Provide</h2>
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                { title: "Referral Analytics", points: ["Service area resource utilization", "Referral pathway mapping", "Community navigation trends"] },
                { title: "Gap Analysis", points: ["County-level shortage identification", "Social determinant mapping", "Service desert detection"] },
                { title: "Community Benefit ROI", points: ["CHNA data integration", "IRS Schedule H alignment", "Population health benchmarking"] },
              ].map((col) => (
                <div key={col.title} className="rounded-lg border border-border p-4 space-y-2">
          ...
          {/* CHNA Workflow */}
          <section>
            <h2 className="text-lg font-bold text-foreground mb-3 border-l-4 border-michigan-teal pl-3">CHNA Integration Workflow</h2>
            <div className="flex flex-wrap gap-2 justify-center">
              {["Data Ingestion", "Needs Mapping", "Gap Analysis", "Report Generation", "Continuous Monitoring"].map((step, i) => (
                <div key={step} className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5 rounded-full border border-michigan-teal/30 bg-michigan-teal/5 px-3 py-1.5">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-michigan-teal text-white text-xs font-bold">{i + 1}</span>
                    <span className="text-xs font-medium text-foreground">{step}</span>
                  </div>
                  {i < 4 && <span className="text-michigan-teal/40 text-lg">→</span>}
                </div>
              ))}
            </div>
          </section>
          ...
          {/* Key Metrics */}
          <section>
            <h2 className="text-lg font-bold text-foreground mb-3 border-l-4 border-michigan-teal pl-3">Platform Impact</h2>
            <div className="grid grid-cols-4 gap-3 text-center">
              {[
                ...
                <div key={m.label} className="rounded-lg border border-border p-3">
                  <p className="text-xl font-bold text-michigan-teal">{m.value}</p>
          ...
          {/* Standards */}
          <section>
            <h2 className="text-lg font-bold text-foreground mb-3 border-l-4 border-michigan-teal pl-3">Standards & Compliance</h2>
            <div className="flex flex-wrap gap-2">
              {["FHIR R4", "HL7 v2", "USCDI v3", "IRS Schedule H", "HIPAA Compliant", "No PHI Stored", "No Cookies", "No Ads"].map((s) => (
