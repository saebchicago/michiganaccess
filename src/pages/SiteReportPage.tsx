...
function Note({ children }: { children: React.ReactNode }) {
  return (
    <div className="border-l-4 border-primary/40 bg-primary/5 px-3 py-2 text-xs text-muted-foreground my-2 print:break-inside-avoid">
      <span className="font-semibold text-primary mr-1">ℹ Interactive element note:</span>
      {children}
    </div>
  );
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mb-8 print:break-inside-avoid-page">
      <h2 className="text-lg font-bold border-b border-border pb-1 mb-3">{title}</h2>
      {children}
    </section>
  );
}
...
export default function SiteReportPage() {
  ...
  const { data: metrics = [], isLoading: ml } = useQualityMetrics(facilityIds.length > 0 ? facilityIds : undefined);
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 text-sm leading-relaxed print:px-0 print:py-0 print:max-w-none">
      ...
      <div className="print:hidden mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Michigan Health Access — Full Site Report</h1>
      ...
      <div className="hidden print:block mb-6">
        <h1 className="text-xl font-bold">Michigan Health Access — Comprehensive Site Report</h1>
      ...
      <Section title="1. Site Overview & Architecture">
        <p>Michigan Health Access is a public civic-infrastructure platform providing free, transparent access to healthcare facility data, provider directories, community resources, financial assistance programs, and quality metrics across the state of Michigan.</p>
        ...
        <table className="w-full text-xs border border-border">
          <thead><tr className="bg-muted"><th className="p-1 text-left border-b">Path</th><th className="p-1 text-left border-b">Page</th></tr></thead>
          <tbody>
            {[
              ["/", "Home"],
      ...
      <Section title="2. Home Page">
        <p>The home page features a hero section with a Michigan landscape image, statistics bar, featured health topics, quick-action cards, how-it-helps explainer, and trust indicators.</p>
        <Note>Hero section contains a search input that filters topics. Quick-action cards are clickable links to internal pages. Stats bar shows aggregate counts from the database.</Note>
      </Section>
          ...
          <tbody>
            {facilities.map((f) => (
              <tr key={f.id} className="border-b border-border/30">
                ...
                <td className="p-1">{f.services?.slice(0, 3).join(", ")}{f.services && f.services.length > 3 ? "…" : ""}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <tbody>
            {providers.map((p) => (
              <tr key={p.id} className="border-b border-border/30">
                <td className="p-1 font-medium">{p.first_name} {p.last_name}, {p.title || "MD"}</td>
                <td className="p-1">{p.specialty}{p.subspecialty ? ` (${p.subspecialty})` : ""}</td>
                <td className="p-1">{p.patient_rating ? Number(p.patient_rating).toFixed(1) : "–"}</td>
                ...
                <td className="p-1">{p.facility_id ? facilityMap[p.facility_id]?.name ?? "–" : "–"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Section title={`6. Financial Help — Programs (${programs.length} total)`}>
          ...
          <tbody>
            {programs.map((p) => (
              <tr key={p.id} className="border-b border-border/30">
                ...
                <td className="p-1">{p.fpl_threshold ? `${p.fpl_threshold}%` : "–"}</td>
        ...
        {programs.map((p) => (
          <div key={p.id} className="mt-2 text-[10px] print:break-inside-avoid">
            ...
            {p.how_to_apply && <p><strong>How to apply:</strong> {p.how_to_apply}</p>}
            {p.services_covered && p.services_covered.length > 0 && (
              <p><strong>Services:</strong> {p.services_covered.join(", ")}</p>
            )}
          </div>
        ))}
      </Section>
      ...
      <Section title={`7. Quality Ratings — Metrics (${metrics.length} total)`}>
          ...
          <tbody>
            {metrics.slice(0, 200).map((m) => (
              <tr key={m.id} className="border-b border-border/30">
                <td className="p-1">{facilityMap[m.facility_id]?.name ?? m.facility_id.slice(0, 8)}</td>
                <td className="p-1">{m.metric_name}</td>
                <td className="p-1">{m.metric_type}</td>
                <td className="p-1">{m.value ?? "–"}{m.unit ? ` ${m.unit}` : ""}</td>
        ...
        {metrics.length > 200 && <p className="text-[10px] text-muted-foreground mt-1">Showing first 200 of {metrics.length} metrics.</p>}
      </Section>

      {/* ── 8. COMMUNITY RESOURCES ── */}
      <Section title={`8. Community Resources (${resources.length} total)`}>
        <Note>The live site has filter tabs by resource type ({resourceTypes.join(", ") || "various types"}), search input, and expandable cards with contact info, hours, eligibility, and services offered.</Note>
        <table className="w-full text-[10px] border border-border">
          <thead>
            <tr className="bg-muted">
      ...
      <Section title="9. Health Conditions">
        <p>Static informational page providing health condition overviews, symptoms, and links to relevant care options within the platform. Content is curated editorial material, not database-driven.</p>
        <Note>The live page has expandable accordion sections for each condition, with internal links to Find Care filtered by relevant specialty.</Note>
      </Section>

      {/* ── 10. INTERACTIVE ELEMENTS SUMMARY ── */}
      <Section title="10. Interactive Elements Summary">
        <p className="mb-2">Comprehensive list of all interactive UI elements across the site:</p>
        <table className="w-full text-[10px] border border-border">
          <thead>
            <tr className="bg-muted">
          ...
          <tbody>
            {[
              ["Home", "Search input", "Text input", "Filters featured topics in real-time"],
              ["Home", "Quick action cards", "Link cards", "Navigate to internal pages"],
              ["Home", "Stats bar", "Display", "Shows aggregate DB counts"],
              ["Find Care", "Facility type filter", "Dropdown (Select)", "Filters facilities by type"],
              ["Find Care", "County filter", "Dropdown (Select)", "Filters facilities by county"],
              ["Find Care", "Search input", "Text input", "Searches facility names"],
              ["Find Care", "Telehealth toggle", "Switch", "Filters telehealth-enabled facilities"],
              ["Find Care", "Walk-in toggle", "Switch", "Filters walk-in facilities"],
              ["Find Care", "Accepting patients toggle", "Switch", "Filters accepting-new-patients"],
              ["Find Care", "Facility cards", "Clickable cards", "Shows facility details"],
              ["Find Care", "Provider search", "Text input", "Searches providers by name/specialty"],
              ["Find Care", "Specialty filter", "Dropdown (Select)", "Filters providers by specialty"],
              ["Find Care", "Language filter", "Dropdown (Select)", "Filters providers by language"],
              ["Health Map", "Map canvas", "Leaflet map", "Pan/zoom, click pins for popups"],
              ["Health Map", "Layer toggles", "Checkboxes", "Show/hide facility type layers"],
              ["Health Map", "Legend", "Display panel", "Color key for facility types"],
              ["Financial Help", "Program type tabs", "Tab buttons", "Filter programs by type"],
              ["Financial Help", "Program cards", "Expandable cards", "Show/hide full details"],
              ["Quality", "Metric type filter", "Dropdown/tabs", "Filter metrics by type"],
              ["Quality", "Facility selector", "Dropdown (Select)", "Filter metrics by facility"],
              ["Quality", "Charts", "Recharts", "Hover for tooltips, compare to averages"],
              ["Resources", "Resource type tabs", "Tab buttons", "Filter resources by type"],
              ["Resources", "Search input", "Text input", "Search resource names"],
              ["Resources", "Resource cards", "Expandable cards", "Show contact/eligibility details"],
              ["Conditions", "Condition accordions", "Accordion", "Expand/collapse condition info"],
              ["All pages", "Header navigation", "Nav links", "Route to all pages"],
              ["All pages", "Footer links", "Link list", "Route to pages + external links"],
              ["All pages", "Mobile menu", "Sheet/drawer", "Hamburger menu on mobile"],
      ...
      <Section title="11. Data Schema Summary">
        <p className="mb-2">All database tables are public read-only (RLS: SELECT only, no writes). No user authentication or personal data collection.</p>
        {[
          { name: "facilities", cols: "id, name, facility_type, address, city, county, state, zip, phone, website, hours, latitude, longitude, quality_score, services[], specialties[], insurance_accepted[], languages[], telehealth_available, walk_in, accepting_new_patients, wheelchair_accessible, public_transit, system_affiliation, leapfrog_grade, is_magnet, is_blue_distinction, joint_commission, digital_capabilities" },
          { name: "providers", cols: "id, first_name, last_name, title, specialty, subspecialty, facility_id→facilities, patient_rating, board_certified, accepting_new_patients, telehealth_available, years_experience, languages[], medical_school, gender, photo_url" },
          { name: "quality_metrics", cols: "id, facility_id→facilities, metric_name, metric_type, value, unit, state_average, national_average, period, data_source" },
          { name: "financial_programs", cols: "id, program_name, program_type, organization, description, eligibility_criteria (JSON), fpl_threshold, how_to_apply, phone, application_url, services_covered[], coverage_area, is_active" },
          { name: "community_resources", cols: "id, resource_name, resource_type, organization, description, address, city, county, state, zip, phone, website, hours, eligibility_notes, languages[], is_free, accepts_insurance, walk_in_available, services_offered[], is_active" },
        ].map((t) => (
          <div key={t.name} className="mb-2 text-[10px] print:break-inside-avoid">
      ...
      <div className="text-center text-[10px] text-muted-foreground border-t border-border pt-4 mt-8">
        <p>Michigan Health Access — Site Report — Generated {new Date().toLocaleString()}</p>
