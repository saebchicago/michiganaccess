...
describe("MedicaidCoverageAtRiskPage", () => {
  ...
  it("table has sortable column headers for all four columns", () => {
    ...
    expect(screen.getByRole("button", { name: /projected loss low/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /projected loss high/i })).toBeInTheDocument();
  });
  ...
  it("column headers have 'modeled range' sub-labels", () => {
    renderPage();
    const subLabels = screen.getAllByText(/modeled range — not a point estimate/i);
  ...
  it("methodology link points to /methodology/medicaid-coverage-at-risk", () => {
    ...
    const methodLinks = screen.getAllByRole("link").filter(
      (el) => el.getAttribute("href") === "/methodology/medicaid-coverage-at-risk"
    );
