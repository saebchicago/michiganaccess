...
import { Activity, GitBranch, Database, HeartPulse, TrendingUp, Info, ExternalLink } from "lucide-react";
...
const leanItems = [
  {
    ...
    metrics: { before: "4.5 hours", after: "12 minutes", improvement: "94%", label: "Projected Waste Elimination" },
    ...
    footnote: "Navigation time reduction is a modeled projection based on reducing 5 search handoffs to 1. Comparable Lean interventions show lead time reductions of 60–88% (Holden, 2011; PMC systematic review). ED visit cost range per AHRQ HCUP StatBrief #268 and Illustra Health (2025). Diversion volume is a platform target.",
  },
  {
    icon: GitBranch,
    category: "Leakage Reduction",
    question: "How does centralized SDOH navigation preserve network revenue?",
    metrics: { before: "34%", after: "67%", improvement: "+97%", label: "Projected Referral Completion" },
    roi: "Projected: $2.4M in preserved network revenue annually through reduced out-of-network referrals",
    detail: "Integrating community resources into the clinical referral workflow keeps patients within coordinated care ecosystems. Addresses housing, food, and transportation barriers that drive out-of-network exits.",
    footnote: "34% baseline aligns with pooled meta-analysis of SDOH referral completion rates (mean 36%, Kreuter et al., PMC). 67% target based on CDC evidence showing direct/warm-handoff referral models raising connection rates from 5% to 75%. Revenue preservation is modeled based on estimated network leakage per out-of-network referral.",
  },
  {
    icon: Database,
    ...
    footnote: "CMS Hospital Compare refreshes quarterly (Jan, Apr, Jul, Oct) per eCQI.healthit.gov. HRSA UDS submissions are annual (due Feb 15), published mid-year per BPHC.HRSA.gov. MDHHS feed cadence varies by program.",
  },
  {
    icon: HeartPulse,
...
const wasteTable = [
  { metric: "Search handoffs", before: "5", after: "1", icon: "🔄", source: "Platform design (handoff reduction model)" },
  { metric: "Navigation time", before: "4.5 hrs", after: "12 min", icon: "⏱️", source: "Projected based on handoff elimination" },
  { metric: "Data sources consulted", before: "8+", after: "1 (unified)", icon: "📊", source: "Platform architecture" },
  { metric: "Referral completion", before: "34%", after: "67%", icon: "🎯", source: "PMC meta-analysis baseline; CDC warm-handoff target" },
  { metric: "Resource freshness verification", before: "Manual", after: "Automated", icon: "✅", source: "CMS + HRSA UDS + MDHHS cross-validation" },
];
      ...
      <TooltipProvider>
        <section className="py-16">
          <div className="container max-w-5xl">
            ...
            <Card className="mb-8 border-primary/15 bg-primary/5">
              <CardContent className="py-4">
                <p className="text-xs text-muted-foreground leading-relaxed flex items-start gap-2">
                  ...
                  <span>
                    <strong className="text-foreground">Methodology note:</strong> Performance metrics on this page are <em>projected targets</em> based on Lean intervention benchmarks from peer-reviewed literature, not achieved outcomes. All figures are labeled accordingly with source citations. Cost ranges reflect published data from AHRQ, CMS, and HRSA.
                  </span>
                </p>
              </CardContent>
            </Card>
                  ...
                  <table className="w-full text-sm min-w-[500px]">
                    <thead>
                      <tr className="border-b">
                        ...
                        <th className="py-2 text-center text-xs text-muted-foreground font-medium">Current State</th>
                        <th className="py-2 text-center text-xs text-muted-foreground font-medium">Projected State</th>
                        <th className="py-2 text-right text-xs text-muted-foreground font-medium">Source / Basis</th>
                      </tr>
                    </thead>
                    <tbody>
                      {wasteTable.map((r) => (
                        <tr key={r.metric} className="border-b last:border-0">
                          ...
                          <td className="py-2 text-right text-[10px] text-muted-foreground max-w-[180px]">{r.source}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <CardContent className="py-5 space-y-4">
                      ...
                      <div className="grid sm:grid-cols-2 gap-3">
                        <div className="rounded-lg bg-muted/50 p-3">
                          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Projected Performance</p>
                          <p className="text-xs">
                            <span className="text-destructive">{item.metrics.before}</span> → <span className="text-primary font-bold">{item.metrics.after}</span>
                          </p>
                        ...
                        <div className="rounded-lg bg-primary/5 p-3">
                          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Projected ROI Impact</p>
            ...
            <Card className="mb-10">
              <CardContent className="py-5">
                <h2 className="text-sm font-bold text-foreground mb-3">References & Data Sources</h2>
                <ul className="space-y-1.5">
                  {sources.map((s) => (
                    <li key={s.url} className="text-xs text-muted-foreground flex items-start gap-1.5">
