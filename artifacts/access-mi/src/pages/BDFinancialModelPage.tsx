...
const COUNTIES: County[] = [
  ...
  { name: "Iosco", opp: 70, shortage: 2.8, svi: 0.7, pop: 24800 },
  ...
  { name: "Bay", opp: 52, shortage: 1.8, svi: 0.5, pop: 103000 },
...
const BPA_TABLE = [
  ...
  },
  {
    dim: "5-yr NPV (base)",
    build: "$22M",
    partner: "$16M (50% stake)",
    acquire: "$28M",
    highlight: true,
  },
  { dim: "Integration risk", build: "Low", partner: "Medium", acquire: "High" },
  {
    dim: "Market share gain",
    build: "Gradual",
    partner: "Moderate",
    acquire: "Immediate",
  },
  {
    dim: "Mission alignment",
    build: "High",
    partner: "High",
    acquire: "Variable",
  },
  {
    dim: "Best for",
    build: "New service lines with differentiation potential",
    partner: "Geography / payer access constraints",
    acquire: "Established competitors with retained revenue",
    highlight: true,
  },
];
...
const SDOH_PRESETS: { label: string; values: SdohState }[] = [
  {
    label: "Henry Ford Health (Detroit)",
    values: { pop: 180000, scr: 72, pos: 17, nav: 34, cst: 14500, rr: 16 },
  },
  {
    label: "Trinity Health (Statewide)",
    values: { pop: 120000, scr: 70, pos: 27, nav: 35, cst: 14500, rr: 16 },
  },
  {
    label: "Corewell West Michigan",
    values: { pop: 95000, scr: 58, pos: 14, nav: 28, cst: 13200, rr: 14 },
  },
];
...
function hasSavedDiff<T extends Record<string, unknown>>(
  key: string,
  defaults: T,
): boolean {
  try {
    ...
    return Object.keys(defaults).some(
      (k) =>
        parsed[k] !== undefined &&
        parsed[k] !== (defaults as Record<string, unknown>)[k],
    );
  } catch {
    return false;
  }
...
function calcNPV(s: ServiceLineState): number {
  const pm =
    (s.commercial / 100) * PAYER_MULTIPLIERS.commercial +
...
function ServiceLineTab({ sl, onSl }: ServiceLineTabProps) {
  ...
  const result = useMemo(() => {
    const pm =
      (sl.commercial / 100) * PAYER_MULTIPLIERS.commercial +
    ...
    const y1net =
      chartData[0].revenue * 1e6 * (1 - sl.opc / 100) - startup * 0.1;
  ...
  const tornadoData = useMemo(() => {
    ...
    const items: { variable: string; hiDelta: number; loDelta: number }[] = [
      ...
      },
      {
        ...
        hiDelta:
          (calcNPV({
            ...sl,
            commercial: Math.min(100, sl.commercial + 10),
            selfPay: Math.max(0, sl.selfPay - 10),
          }) -
            base) /
          1e6,
        loDelta:
          (calcNPV({
            ...sl,
            commercial: Math.max(0, sl.commercial - 10),
            selfPay: Math.min(100, sl.selfPay + 10),
          }) -
            base) /
          1e6,
      },
    ...
    return items
      .sort(
        (a, b) =>
          Math.abs(b.hiDelta - b.loDelta) - Math.abs(a.hiDelta - a.loDelta),
      )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KPI
          label="5-year NPV"
          value={fmtM(result.npv)}
          sub="at 8% discount rate"
          good={result.npv > 0}
        />
        <KPI
          label="Year 1 net"
          value={fmtM(result.y1net)}
          sub="after startup costs"
          good={result.y1net > 0}
        />
        <KPI
          label="Breakeven"
          value={result.be ? `Year ${result.be}` : ">5 yrs"}
          sub="cumulative cash flow"
        />
        <KPI
          label="Market capture Yr 1"
          value={`${result.ms.toFixed(1)}%`}
          sub="of addressable market"
        />
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div>
          ...
          <SliderRow
            label="Annual patient volume (Yr 1)"
            value={sl.vol}
            display={sl.vol.toLocaleString()}
            min={500}
            max={20000}
            step={100}
            onChange={(v) => onSl({ vol: v })}
          />
          <SliderRow
          ...
          <div className="mb-2 mt-1">
            ...
            <SliderRow
              label="Commercial (%)"
              value={sl.commercial}
              display={`${sl.commercial}%`}
              min={0}
              max={100}
              step={1}
              onChange={(v) => onSl({ commercial: v })}
            />
            <SliderRow
              label="Medicare (%)"
              value={sl.medicare}
            ...
            <SliderRow
              ...
              max={100}
              step={1}
              onChange={(v) => onSl({ selfPay: v })}
            />
          </div>

          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4 mt-4">
            Cost structure
          </p>
          <SliderRow
        ...
        <div>
          ...
          <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-sm bg-[#378ADD33] border border-[#378ADD] inline-block" />
              Revenue
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-sm bg-[#1D9E75] inline-block" />
          ...
          {/* Market insight card */}
          <div
            className={`mt-4 rounded-lg border-l-2 px-4 py-3 transition-colors ${aiLoading ? "border-muted bg-muted/20" : "border-michigan-teal bg-muted/40"}`}
          >
      ...
      <div className="rounded-lg border border-border p-4">
        ...
        <ResponsiveContainer width="100%" height={180}>
          ...
          >
            ...
            <Bar
              dataKey="favorable"
              name="Favorable impact"
              fill="#1D9E75"
              radius={[0, 3, 3, 0]}
            />
            <Bar
              dataKey="unfavorable"
              name="Unfavorable impact"
              fill="#D85A30"
              radius={[3, 0, 0, 3]}
            />
          </BarChart>
        ...
        <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-[#D85A30] inline-block" />
            Unfavorable (−10%)
          </span>
        </div>
      </div>
...
function BuildPartnerAcquireTab() {
  const [selected, setSelected] = useState<"build" | "partner" | "acquire">(
    "build",
  );
  ...
  const cards = [
    {
      ...
      desc: "Full ownership · highest control · longest runway",
    },
    {
      id: "partner" as const,
      badge: "Partner / JV",
      color: "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300",
      desc: "Shared capital · faster launch · aligned incentives",
    },
    {
      id: "acquire" as const,
      badge: "Acquire",
      ...
      desc: "Immediate volume · highest upfront cost · integration risk",
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {cards.map((c) => (
          ...
          >
            <span
              className={`inline-block rounded-md px-2 py-0.5 text-xs font-semibold mb-2 ${c.color}`}
            >
              {c.badge}
            </span>
            <p className="text-xs text-muted-foreground leading-relaxed">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              <th className="text-left p-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground w-40">
                Dimension
              </th>
              <th className="text-left p-3 text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
                Build
              </th>
              <th className="text-left p-3 text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400">
                Partner / JV
              </th>
              <th className="text-left p-3 text-xs font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400">
                Acquire
              </th>
            </tr>
          </thead>
          <tbody>
            {BPA_TABLE.map((row) => (
              <tr
                key={row.dim}
                className={`border-b border-border/60 ${row.highlight ? "bg-muted/30" : ""}`}
              >
      ...
      <div>
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
          Risk-adjusted NPV by scenario ($M)
        </p>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart
            data={BPA_CHART}
        ...
        <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-[#378ADD] inline-block" />
            Base case
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-sm bg-[#D85A30] inline-block" />
...
function MarketOpportunityTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === "shortage")
      return [...data].sort((a, b) => b.shortage - a.shortage);
  ...
  const handleCountyClick = useCallback(async (county: County) => {
    ...
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a healthcare BD analyst specializing in Michigan market analysis.",
            },
      ...
      setCountyAI(
        "AI analysis unavailable. Please check your connection and try again.",
      );
    } finally {
      setAiLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <KPI
          label="Avg shortage ratio"
          value="3.2×"
          sub="need vs. supply index"
        />
        <KPI
          label="Avg SVI (top tier)"
          value="0.74"
          sub="CDC vulnerability index"
        />
      </div>
      ...
      <div className="space-y-1">
        {filtered.length === 0 && (
          <p className="text-sm text-muted-foreground py-4 text-center">
            No counties meet the minimum score threshold.
          </p>
        )}
        {filtered.map((c) => {
          const barW = Math.round((c.opp / maxOpp) * 100);
          const color =
            c.opp >= 80 ? "#1D9E75" : c.opp >= 65 ? "#378ADD" : "#BA7517";
          ...
          <>
            <div className="flex items-center gap-2 mb-3">
              <p className="text-xs font-semibold text-foreground">
                {selected.name} County - AI Analysis
              </p>
              <Badge variant="outline" className="text-xs">
                {selected.opp}/100 opportunity
              </Badge>
            </div>
            {aiLoading && (
              <p className="text-xs text-muted-foreground animate-pulse">
                Generating AI analysis for {selected.name} County…
              </p>
            )}
            {!aiLoading && countyAI && (
      ...
      <p className="text-xs text-muted-foreground leading-relaxed border-t border-border pt-4">
        Opportunity scores derived from CDC Social Vulnerability Index 2022,
        HRSA Health Professional Shortage Area data, and County Health Rankings
        2025. Scores are composite indices for strategic planning - not
...
function SdohImpactTab({ sdoh, onSdoh }: SdohImpactTabProps) {
  ...
  const [progDelivery, setProgDelivery] = useState(
  ...
  const result = useMemo(() => {
    const intervened =
      sdoh.pop * (sdoh.scr / 100) * (sdoh.pos / 100) * (sdoh.nav / 100);
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a healthcare program designer with expertise in SDOH interventions.",
            },
            { role: "user", content: prompt },
          ],
        }),
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground max-w-2xl">
        Model the downstream financial impact of SDOH interventions on
        readmissions, ED utilization, and value-based care contract performance.
        In VBC arrangements - MSSP ACOs, BCBSM Blueprint, Medicare Advantage
        risk - SDOH navigation directly drives shared savings, Stars scores, and
        total cost of care reduction. Anchored to Trinity Health (27.4% unmet
        need from 1M+ screened; system-reported ~16% readmission reduction,
        FY2025 - not independently verified) and Henry Ford Health ACO data.
      </p>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KPI
          label="Avoided readmissions"
          value={fmtM(result.readmitSave)}
          sub="annual savings"
          good={result.readmitSave > 0}
        />
        <KPI
          label="ED diversion"
          value={fmtM(result.edSave)}
          sub="avoided ED costs"
          good={result.edSave > 0}
        />
        <KPI
          label="VBC bonus potential"
          value={fmtM(result.vbcBonus)}
          sub="quality metric uplift"
        />
        <KPI
          label="3-year ROI"
          value={`${result.roi3.toFixed(1)}×`}
          sub="on program cost"
          good={result.roi3 > 2}
        />
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div>
          ...
          <SliderRow
            label="Attributed population (patients/yr)"
            value={sdoh.pop}
            display={sdoh.pop.toLocaleString()}
            min={1000}
            max={100000}
            step={1000}
            onChange={(v) => onSdoh({ pop: v })}
          />
          <SliderRow
            label="SDOH screening rate"
            value={sdoh.scr}
            display={`${sdoh.scr}%`}
            min={10}
            max={95}
            step={5}
            onChange={(v) => onSdoh({ scr: v })}
          />
          <SliderRow
            label="Positive screen rate (unmet need)"
            value={sdoh.pos}
            display={`${sdoh.pos}%`}
            min={5}
            max={40}
            step={1}
            onChange={(v) => onSdoh({ pos: v })}
          />
          <SliderRow
            label="Navigation intervention rate"
            value={sdoh.nav}
            display={`${sdoh.nav}%`}
            min={10}
            max={85}
            step={1}
            onChange={(v) => onSdoh({ nav: v })}
          />
          <SliderRow
            label="Avg cost per readmission"
            value={sdoh.cst}
            display={`$${sdoh.cst.toLocaleString()}`}
            min={8000}
            max={30000}
            step={500}
            onChange={(v) => onSdoh({ cst: v })}
          />
          <SliderRow
            label="Readmission reduction from nav."
            value={sdoh.rr}
            display={`${sdoh.rr}%`}
            min={5}
            max={30}
            step={1}
            onChange={(v) => onSdoh({ rr: v })}
          />
        </div>

        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4">
            3-year cumulative impact ($M)
          </p>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart
              data={result.chartData}
          ...
          <div className="flex gap-4 mt-2 text-xs text-muted-foreground flex-wrap">
            ...
            <span className="flex items-center gap-1.5">
              <span className="h-2.5 w-2.5 rounded-sm bg-[#9FE1CB] border border-[#0F6E56] inline-block" />
              VBC bonus
            </span>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              ...
              },
              {
                ...
                impact: "outcome driver",
                color: "text-muted-foreground",
              },
            ].map((item) => (
              <div
                key={item.label}
                className="rounded-md bg-muted/30 p-2 text-center"
              >
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <Select value={progPop} onValueChange={setProgPop}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Children & Youth">
                  Children & Youth
                </SelectItem>
          ...
          <div>
            ...
            <Select value={progNeed} onValueChange={setProgNeed}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Housing instability">
                  Housing instability
                </SelectItem>
                <SelectItem value="Transportation barriers">
          ...
          <div>
            ...
            <Select value={progDelivery} onValueChange={setProgDelivery}>
              ...
              <SelectContent>
                <SelectItem value="Community health workers (CHW)">
                  Community health workers (CHW)
                </SelectItem>
                <SelectItem value="Telehealth">Telehealth</SelectItem>
                <SelectItem value="In-person clinic">
                  In-person clinic
                </SelectItem>
...
function ExportShareTab({ slState, sdohState }: ExportShareTabProps) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground max-w-2xl">
        Share your current scenario with colleagues, export a summary for
        presentations, or print this page as a PDF. All slider values are
        encoded in the shareable link.
      </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2 text-xs">
          <div className="space-y-1.5">
            ...
            <p className="text-muted-foreground">
              ...
              <span className="text-foreground">
                {slState.vol.toLocaleString()}/yr
              </span>
            </p>
            <p className="text-muted-foreground">
              RPE: <span className="text-foreground">${slState.rpe}</span>
            </p>
            <p className="text-muted-foreground">
              Growth: <span className="text-foreground">{slState.grw}%</span>
            </p>
            <p className="text-muted-foreground">
              Payer mix:{" "}
              <span className="text-foreground">
                {slState.commercial}% Comm / {slState.medicare}% MCR /{" "}
                {slState.medicaid}% MCD / {slState.selfPay}% SP
              </span>
            </p>
            <p className="text-muted-foreground">
              Startup: <span className="text-foreground">${slState.stup}M</span>
            </p>
            <p className="text-muted-foreground">
              Op. cost: <span className="text-foreground">{slState.opc}%</span>
            </p>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-muted-foreground">
              Population:{" "}
              <span className="text-foreground">
                {sdohState.pop.toLocaleString()}
              </span>
            </p>
            <p className="text-muted-foreground">
              Screening:{" "}
              <span className="text-foreground">{sdohState.scr}%</span>
            </p>
            <p className="text-muted-foreground">
              Pos. screen:{" "}
              <span className="text-foreground">{sdohState.pos}%</span>
            </p>
            <p className="text-muted-foreground">
              Navigation:{" "}
              <span className="text-foreground">{sdohState.nav}%</span>
            </p>
            <p className="text-muted-foreground">
              Cost/readmit:{" "}
              <span className="text-foreground">
                ${sdohState.cst.toLocaleString()}
              </span>
            </p>
            <p className="text-muted-foreground">
              Readmit reduction:{" "}
              <span className="text-foreground">{sdohState.rr}%</span>
            </p>
          </div>
        </div>
      </div>
...
export default function BDFinancialModelPage() {
  usePageMeta({
    ...
    description:
      "Interactive partnership and market development scenario modeler. Service line NPV, build vs. partner vs. acquire comparisons, market opportunity scoring, and SDOH ROI - built on Michigan public health data.",
  ...
  const [sl, setSl] = useState<ServiceLineState>(() => {
    ...
    return {
      vol:
        searchParams.get("vol") !== null
          ? Number(searchParams.get("vol"))
          : stored.vol,
      rpe:
        searchParams.get("rpe") !== null
          ? Number(searchParams.get("rpe"))
          : stored.rpe,
      grw:
        searchParams.get("grw") !== null
          ? Number(searchParams.get("grw"))
          : stored.grw,
      commercial:
        searchParams.get("comm") !== null
          ? Number(searchParams.get("comm"))
          : stored.commercial,
      medicare:
        searchParams.get("medc") !== null
          ? Number(searchParams.get("medc"))
          : stored.medicare,
      medicaid:
        searchParams.get("mcaid") !== null
          ? Number(searchParams.get("mcaid"))
          : stored.medicaid,
      selfPay:
        searchParams.get("sp") !== null
          ? Number(searchParams.get("sp"))
          : stored.selfPay,
      stup:
        searchParams.get("stup") !== null
          ? Number(searchParams.get("stup"))
          : stored.stup,
      opc:
        searchParams.get("opc") !== null
          ? Number(searchParams.get("opc"))
          : stored.opc,
      mktv:
        searchParams.get("mktv") !== null
          ? Number(searchParams.get("mktv"))
          : stored.mktv,
    };
  });
  ...
  const [sdoh, setSdoh] = useState<SdohState>(() => {
    ...
    const hasAnyUrlParam = ["pop", "scr", "pos", "nav", "cst", "rr"].some(
      (k) => searchParams.get(k) !== null,
    );
    if (!hasAnyUrlParam) return stored;
    return {
      pop:
        searchParams.get("pop") !== null
          ? Number(searchParams.get("pop"))
          : stored.pop,
      scr:
        searchParams.get("scr") !== null
          ? Number(searchParams.get("scr"))
          : stored.scr,
      pos:
        searchParams.get("pos") !== null
          ? Number(searchParams.get("pos"))
          : stored.pos,
      nav:
        searchParams.get("nav") !== null
          ? Number(searchParams.get("nav"))
          : stored.nav,
      cst:
        searchParams.get("cst") !== null
          ? Number(searchParams.get("cst"))
          : stored.cst,
      rr:
        searchParams.get("rr") !== null
          ? Number(searchParams.get("rr"))
          : stored.rr,
    };
  });
  ...
  const [showBanner, setShowBanner] = useState<boolean>(() => {
    ...
    const hasUrlParams = ["vol", "rpe", "pop", "scr"].some(
      (k) => searchParams.get(k) !== null,
    );
    ...
    return (
      ...
      hasSavedDiff(
        SDOH_KEY,
        SDOH_DEFAULTS as unknown as Record<string, unknown>,
      )
    );
  });
  ...
  useEffect(() => {
    writeStorage(SL_KEY, sl);
  }, [sl]);
  useEffect(() => {
    writeStorage(SDOH_KEY, sdoh);
  }, [sdoh]);

  const updateSl = useCallback(
    (patch: Partial<ServiceLineState>) =>
      setSl((prev) => ({ ...prev, ...patch })),
    [],
  );
  const updateSdoh = useCallback(
    (patch: Partial<SdohState>) => setSdoh((prev) => ({ ...prev, ...patch })),
    [],
  );
      ...
      <section className="bg-gradient-to-b from-primary/5 to-background py-12 lg:py-16">
        <div className="container max-w-5xl">
          ...
          >
            <Badge
              variant="outline"
              className="mb-4 uppercase tracking-wider text-xs border-primary/30 text-primary"
            >
          ...
          >
            Quantify partnership value, compare growth strategies, and identify
            market opportunity across Michigan's 83 counties - built on CDC SVI,
            HRSA shortage area, and County Health Rankings data.
          </motion.p>
          <motion.div
            initial="hidden"
            animate="visible"
      ...
      <section className="container max-w-5xl py-10">
        ...
        >
          <Tabs defaultValue="service-line">
            <TabsList className="flex-wrap h-auto gap-1 mb-8">
              <TabsTrigger value="service-line" className="gap-1.5">
                <Calculator className="h-3.5 w-3.5" />
                Service line model
              </TabsTrigger>
              <TabsTrigger value="bpa" className="gap-1.5">
                <GitBranch className="h-3.5 w-3.5" />
                Build · Partner · Acquire
              </TabsTrigger>
              <TabsTrigger value="market-opp" className="gap-1.5">
                <Map className="h-3.5 w-3.5" />
                Market opportunity
              </TabsTrigger>
              <TabsTrigger value="sdoh-impact" className="gap-1.5">
            ...
            <TabsContent value="service-line">
              <ServiceLineTab sl={sl} onSl={updateSl} />
            </TabsContent>
            <TabsContent value="bpa">
              <BuildPartnerAcquireTab />
            </TabsContent>
            <TabsContent value="market-opp">
              <MarketOpportunityTab />
            </TabsContent>
            <TabsContent value="sdoh-impact">
              <SdohImpactTab sdoh={sdoh} onSdoh={updateSdoh} />
            </TabsContent>
            <TabsContent value="export">
              <ExportShareTab slState={sl} sdohState={sdoh} />
            </TabsContent>
          </Tabs>
        </motion.div>
        ...
        >
          <p className="text-xs font-semibold text-foreground mb-2 flex items-center gap-2">
            <Info className="h-3.5 w-3.5 text-muted-foreground" /> Data sources
            &amp; methodology
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Financial projections are projected scenario models for strategic
            planning purposes. Market opportunity scores derived from{" "}
            <strong className="text-foreground">
              CDC Social Vulnerability Index 2022
            </strong>
            ,{" "}
            <strong className="text-foreground">
              HRSA Health Professional Shortage Area
            </strong>{" "}
            data, and{" "}
            <strong className="text-foreground">
              County Health Rankings 2025
            </strong>
            . SDOH impact parameters anchored to published outcomes from{" "}
            <strong className="text-foreground">Trinity Health</strong> (27.4%
            unmet social need rate from 1M+ screened; system-reported ~16%
            hospitalization reduction, FY2025 - not independently verified) and
            <strong className="text-foreground">Henry Ford Health</strong> ACO
            shared savings data (PY2024 $19.97M). Not actuarial projections.
            AI-generated narratives are for strategic planning illustration
            only.
          </p>
        </motion.div>
      </section>
