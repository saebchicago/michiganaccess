...
const scalabilityMetrics = [
  ...
  {
    labelKey: "countiesCovered",
    value: String(COUNTIES_COVERED),
    desc: "All Michigan counties",
  },
  {
    labelKey: "providersCount",
    value: "20,000+",
    desc: "Facilities + individual providers",
  },
];

const growthPlan = [
  {
    titleKey: "horizontalScaling",
    desc: "Connection pooling + read replicas for high-traffic periods",
    icon: Server,
  },
  {
    titleKey: "geoCDN",
    desc: "CDN distribution for statewide latency <100ms",
    icon: Globe,
  },
  {
...
export default function TechnicalPage() {
  ...
  usePageMeta({
    title: t("technicalPage.badge"),
    description: t("technicalPage.subtitle"),
    path: "/technical",
  });
  return (
    <Layout>
      ...
      <section className="bg-gradient-to-b from-michigan-navy/5 to-background py-16 lg:py-24">
        <div className="container max-w-4xl text-center">
          ...
          <motion.span
            ...
            variants={fade}
            custom={0}
            className="mb-4 inline-block rounded-full bg-michigan-navy/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-michigan-navy"
          >
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h2 className="text-2xl font-bold text-foreground">
                  {t("technicalPage.archTitle")}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {t("technicalPage.archSubtitle")}
                </p>
              </div>
            </div>
          </motion.div>
                ...
                <Card className="overflow-hidden">
                  <CardContent className="p-0">
                    <div className="flex flex-col sm:flex-row">
                      <div
                        className={`flex items-center gap-3 px-5 py-4 sm:w-56 ${layer.color}`}
                      >
                        <layer.icon className="h-5 w-5" />
                        <span className="text-sm font-bold">
                          {t(`technicalPage.${layer.titleKey}`)}
                        </span>
                      </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {securityItems.map((s) => (
                <div
                  ...
                  className="flex items-start gap-2 text-xs text-muted-foreground"
                >
                  <s.icon className="mt-0.5 h-3.5 w-3.5 shrink-0 text-michigan-forest-deep" />
                  {s.text}
                </div>
              ))}
            </div>
        ...
        <section>
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h2 className="text-2xl font-bold text-foreground">
                  {t("technicalPage.scalabilityTitle")}
                </h2>
                <p className="text-sm text-muted-foreground">
                  {t("technicalPage.scalabilitySubtitle")}
                </p>
              </div>
            </div>
          </motion.div>
              ...
              >
                <Card className="h-full hover-lift">
                  <CardContent className="pt-6">
                    <g.icon className="mb-3 h-5 w-5 text-michigan-teal-deep" />
                    <h4 className="mb-1 text-sm font-bold text-foreground">
                      {t(`technicalPage.${g.titleKey}`)}
                    </h4>
