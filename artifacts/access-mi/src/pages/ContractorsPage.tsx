      ...
      <section className="relative overflow-hidden bg-slate-900 py-12 md:py-16">
        <div className="container">
          ...
          >
            ...
            >
              Every federal contract, every award, every recipient - reported by
              law under the DATA Act. $14.2B flowed to Michigan in FY2024.
            </motion.p>
            <motion.p
              variants={fadeUp}
              custom={3}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Search className="h-4 w-4 text-primary" /> Search by County
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Select a county to see top federal contract recipients. Live data
          ...
          <CardContent>
            <div className="flex gap-2 mb-4">
              <Select
                value={county ?? "all"}
                onValueChange={(v) => setCounty(v === "all" ? undefined : v)}
              >
                <SelectTrigger className="w-52">
                  <SelectValue placeholder="All Michigan" />
                </SelectTrigger>
                <SelectContent>
            ...
            {contractors && contractors.length > 0 && (
              <>
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 mb-4">
                  <div className="rounded-lg border border-border p-3 text-center">
                    <p className="text-xl font-bold text-amber-600 tabular-nums">
                      ${(totalAwarded / 1e6).toFixed(1)}M
                    </p>
                    <p className="text-[10px] text-muted-foreground">
                      Total Awarded (top results)
                    </p>
                  </div>
                  <div className="rounded-lg border border-border p-3 text-center">
                    ...
                    <tbody>
                      {contractors.map((c, i) => (
                        ...
                        >
                          ...
                          <td className="py-2 pr-4 tabular-nums font-semibold text-amber-600">
                            ${(c.totalAwardsFY2024 / 1e6).toFixed(2)}M
                          </td>
                ...
                <p className="text-[9px] text-muted-foreground/60 mt-2">
                  Source: USASpending.gov FY2024. Live API data - results may
                  vary. All contracts $10,000+ reported under FFATA.
                </p>
              </>
            )}
            {contractors && contractors.length === 0 && !isLoading && (
              <p className="text-sm text-muted-foreground py-6">
                No contract results found. Try a different county or check
                USASpending.gov directly.
              </p>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="py-4">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <p>
                  All federal contracts $10,000+ must be reported under the
                ...
                <div className="flex gap-3 mt-2">
                  <a
                    ...
                    aria-label="USASpending.gov, opens in new window"
                  >
                    USASpending.gov{" "}
                ...
                <p className="mt-1">
                  Verify a contractor&apos;s federal registration at{" "}
                  <a
                    href="https://sam.gov"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-primary hover:underline inline-flex items-center gap-1"
                    aria-label="SAM.gov, opens in new window"
                  >
                    SAM.gov{" "}
                    <ExternalLink className="h-3 w-3" aria-hidden="true" />
                  </a>
