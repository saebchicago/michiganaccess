...
const CRISIS_988_DATA = {
  ...
  primaryCenter:
    "MiCAL (Common Ground, Oakland County) - handles ~85% of calls",
...
const SUD_DATA = [
  {
    metric: "Overdose deaths decline (2023→2024)",
    value: "35%",
    color: "#22c55e",
  },
  ...
  {
    metric: "Counties with zero buprenorphine prescribers",
    value: "20",
    color: "#ef4444",
  },
  {
    metric: "Opioid settlement expected by 2040",
    value: "$1.8B",
    color: "#0A4C95",
  },
  {
    metric: "Communities that haven't spent settlement $",
    value: "43%",
    color: "#f59e0b",
  },
];
...
const NEW_FACILITIES = [
  {
    name: "SE Michigan Psychiatric Hospital",
    location: "Northville",
    beds: 260,
    opening: "Oct 2026",
    type: "Adult",
  ...
  {
    name: "Pine Rest Pediatric Center",
        ...
        <div className="container max-w-3xl text-center">
          <motion.div initial="hidden" animate="visible">
            <motion.div
              variants={fadeUp}
              custom={0}
              className="mb-4 inline-flex items-center gap-2 rounded-full bg-purple-500/10 px-4 py-1.5"
            >
              <Brain className="h-4 w-4 text-purple-600" />
              <span className="text-sm font-medium text-purple-600">
                Michigan's #1 CHNA-Identified Need
              </span>
            </motion.div>
            <motion.h1
              variants={fadeUp}
            ...
            >
              Sources: Treatment Advocacy Center · MDHHS · SAMHSA · Bridge
              Michigan · MHA
            </motion.p>
          </motion.div>
        </div>
        ...
        <div className="container py-6">
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">
                patients boarding in ERs daily
              </p>
              <p className="text-[9px] text-muted-foreground/60">
                MHA survey est. / MDHHS
              </p>
            </div>
            {/* Source: MDHHS 988 */}
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-muted-foreground">
                CCBHCs (2x any other state)
              </p>
              <p className="text-[9px] text-muted-foreground/60">
                SAMHSA Demonstration
              </p>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue="beds" className="w-full">
          <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
            <TabsList className="inline-flex w-max min-w-full sm:w-auto sm:min-w-0 gap-1">
              ...
              >
                <Building2 className="h-3.5 w-3.5" /> Psychiatric Beds
              </TabsTrigger>
              <TabsTrigger
                value="crisis"
                className="text-xs sm:text-sm whitespace-nowrap gap-1.5"
              >
                <Phone className="h-3.5 w-3.5" /> Crisis (988 + CSU)
              </TabsTrigger>
              <TabsTrigger
                value="ccbhc"
                className="text-xs sm:text-sm whitespace-nowrap gap-1.5"
              >
                <Brain className="h-3.5 w-3.5" /> CCBHCs
              </TabsTrigger>
              <TabsTrigger
                value="sud"
                className="text-xs sm:text-sm whitespace-nowrap gap-1.5"
              >
                <Pill className="h-3.5 w-3.5" /> SUD Treatment
              </TabsTrigger>
              <TabsTrigger
                value="children"
                className="text-xs sm:text-sm whitespace-nowrap gap-1.5"
              >
                <Baby className="h-3.5 w-3.5" /> Children & Youth
              </TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="beds" className="mt-6 space-y-6">
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Source: Treatment Advocacy Center · Recommended: 40-60 per
                    100K
                  </p>
                </CardHeader>
                <CardContent>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <Tooltip
                          ...
                          formatter={(v: number) => `${v} per 100K`}
                        />
                        <Bar dataKey="per100k" radius={[0, 4, 4, 0]}>
                          {PSYCH_BED_DATA.map((e) => (
                            <Cell key={e.region} fill={e.color} />
                          ))}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
              ...
              <Card>
                <CardHeader>
                  ...
                  <p className="text-xs text-muted-foreground">
                    1,945 beds vs ~4,000 needed (40/100K minimum)
                  </p>
                </CardHeader>
                <CardContent>
                  <div className="h-[280px]">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {NEW_FACILITIES.map((f) => (
                <Card
                  key={f.name}
                  className="border-teal-200/50 dark:border-teal-900/30 bg-teal-50/30 dark:bg-teal-950/10"
                >
                  <CardContent className="py-4">
                    <Badge className="text-[9px] bg-teal-100 text-teal-700 dark:bg-teal-950/30 dark:text-teal-400 mb-2">
                      Opening {f.opening}
                    </Badge>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {f.location} · {f.beds} {f.type} beds · Source: {f.source}
                    </p>
                  </CardContent>
                </Card>
              ))}
            ...
            <Card className="border-red-200/50 dark:border-red-900/30 bg-red-50/30 dark:bg-red-950/10">
              <CardContent className="py-4">
                <p className="text-xs text-muted-foreground">
                  ...
                  13 of 15 Upper Peninsula counties have zero inpatient
                  psychiatric beds. Zero child psych beds in the entire UP.
                  Source: MDHHS / MHA
                </p>
              </CardContent>
            </Card>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-foreground tabular-nums">
                      {(CRISIS_988_DATA.totalCalls / 1000).toFixed(0)}K
                    </p>
                    <p className="text-xs text-muted-foreground">
                      calls answered
                    </p>
                  ...
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600 tabular-nums">
                      {CRISIS_988_DATA.avgAnswerTimeSec}s
                    </p>
                    <p className="text-xs text-muted-foreground">
                      avg answer time
                    </p>
                  ...
                  <div className="text-center">
                    <p className="text-2xl font-bold text-green-600 tabular-nums">
                      {CRISIS_988_DATA.inStateAnswerRate}%
                    </p>
                    <p className="text-xs text-muted-foreground">
                      in-state answer rate
                    </p>
                    <p className="text-[9px] text-muted-foreground/60">
                      Best among high-volume states
                    </p>
                  </div>
                  <div className="text-center">
                    <p className="text-2xl font-bold text-foreground tabular-nums">
                    ...
                    <p className="text-xs text-muted-foreground">
                      monthly calls (growing)
                    </p>
                  </div>
                </div>
                <p className="text-[9px] text-muted-foreground/60 mt-3">
                  {CRISIS_988_DATA.primaryCenter}. Source:{" "}
                  {CRISIS_988_DATA.source}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  ${(CSU_DATA.funding / 1e6).toFixed(0)}M for{" "}
                  {CSU_DATA.totalPlanned} adult CSUs. Source: MDHHS
                </p>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-foreground">
                      Network 180 (Grand Rapids) + DWIHN (Wayne County)
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant="outline">Planned</Badge>
                    <span className="text-sm text-muted-foreground">
                      {CSU_DATA.totalPlanned - CSU_DATA.operational} additional
                      CSUs in development
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2">
                    When all {CSU_DATA.totalPlanned} built: covers{" "}
                    {CSU_DATA.popCoverageWhenBuilt}% of population.{" "}
                    {CSU_DATA.gapRemaining}% will remain without crisis
          ...
          <TabsContent value="ccbhc" className="mt-6 space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Card>
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-primary tabular-nums">
                    {CCBHC_DATA.clinicCount}
                  </p>
                  <p className="text-xs text-muted-foreground">CCBHCs</p>
                  <p className="text-[9px] text-muted-foreground/60">
                    2x any other state
                  </p>
                </CardContent>
              ...
              <Card>
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-green-600 tabular-nums">
                    {CCBHC_DATA.sudIncrease}%
                  </p>
                  <p className="text-xs text-muted-foreground">
                    SUD individuals served increase
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-green-600 tabular-nums">
                    {CCBHC_DATA.edReduction}%
                  </p>
                  <p className="text-xs text-muted-foreground">
                    ED utilization decrease
                  </p>
                  <p className="text-[9px] text-muted-foreground/60">
                    at original 13 sites
                  </p>
                </CardContent>
              </Card>
            </div>
          ...
          <TabsContent value="sud" className="mt-6 space-y-6">
            ...
            <Card className="border-red-200/50 dark:border-red-900/30 bg-red-50/30 dark:bg-red-950/10">
              <CardContent className="py-4">
                <p className="text-xs text-muted-foreground">
                  <AlertTriangle className="h-3.5 w-3.5 inline mr-1 text-red-500" />
                  Urban counties are 35.6x more likely to have a methadone
                  clinic than rural counties. 20 counties have zero
                  buprenorphine prescribers. Source: MDHHS Opioid Strategy /
                  Bridge Michigan
                </p>
              </CardContent>
            </Card>
            <p className="text-[9px] text-muted-foreground/60">
              Sources: MDHHS Opioid Strategy · Bridge Michigan · Michigan Opioid
              ...
              <Card className="border-red-200/50 dark:border-red-900/30">
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-red-600 tabular-nums">
                    {CHILDREN_DATA.residentialCapacityDrop}%
                  </p>
                  <p className="text-xs text-muted-foreground">
                    child residential capacity drop since 2019
                  </p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-foreground tabular-nums">
                    {CHILDREN_DATA.licensedChildPsychBeds}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    licensed child psych beds
                  </p>
                  <p className="text-[9px] text-muted-foreground/60">
                    {CHILDREN_DATA.upChildPsychBeds} in entire UP
                  </p>
                </CardContent>
              </Card>
              <Card className="border-amber-200/50 dark:border-amber-900/30">
                <CardContent className="py-4 text-center">
                  <p className="text-2xl font-bold text-amber-600 tabular-nums">
                    {CHILDREN_DATA.childrenInEDsWaiting}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    children in EDs waiting for placement (any given day)
                  </p>
                </CardContent>
              </Card>
            </div>
              ...
              <CardContent className="py-4">
                <p className="text-xs text-foreground">
                  <span className="font-bold text-green-700 dark:text-green-400">
                    $
                    {(CHILDREN_DATA.schoolMentalHealthFunding / 1e6).toFixed(0)}
                    M
                  </span>{" "}
                  in school mental health funding (FY2025-26). Youth suicide
                  rates have doubled over the past decade. Source: MDHHS /
                  Michigan Legislature
                </p>
              </CardContent>
            </Card>
            <p className="text-[9px] text-muted-foreground/60">
              Sources: MDHHS · Bridge Michigan · MHA · Michigan Legislature
              FY2025-26 Budget
            </p>
          </TabsContent>
        </Tabs>
      ...
      <section className="container pb-8">
        <Card className="bg-muted/30">
          <CardContent className="py-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs text-muted-foreground">
                All data sourced from Treatment Advocacy Center, MDHHS 988
                Performance Reports, SAMHSA CCBHC Demonstration, MDHHS Opioid
                Strategy, Michigan Hospital Association, and Bridge Michigan
                investigative reporting. Last updated March 2026.
              </p>
            </div>
          </CardContent>
        </Card>
        <div className="mt-4">
          <Button asChild variant="outline">
            <Link to="/data">
              <ArrowLeft className="mr-1.5 h-3.5 w-3.5" /> Back to Data Center
            </Link>
          </Button>
        </div>
      </section>
