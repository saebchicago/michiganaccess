...
function getRegionBg(pct: number): string {
  if (pct >= 85) return "bg-red-50 border-red-200 dark:bg-red-950/20 dark:border-red-900/40";
  if (pct >= 75) return "bg-orange-50 border-orange-200 dark:bg-orange-950/20 dark:border-orange-900/40";
...
const CivicPowerPage = () => {
  ...
  const stateCompData = useMemo(() =>
    [...STATE_UNCONTESTED_COMPARISON]
      ...
      .map(s => ({ ...s, fill: s.state === "Michigan" ? "#dc2626" : s.state === "National Average" ? "#6b7280" : "#0A4C95" })),
  []);
  ...
  return (
    <Layout>
        ...
        <div className="container">
          <motion.div initial="hidden" animate="visible" className="mx-auto max-w-3xl text-center">
            <motion.div variants={fadeUp} custom={0} className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5">
              ...
              <span className="text-sm font-medium text-primary">Civic Power Map</span>
            </motion.div>
            ...
            <motion.div variants={fadeUp} custom={4} className="mt-6 flex flex-wrap justify-center gap-3">
              <Button asChild size="lg">
                <Link to="/civic-power/boards">Find Where to Serve <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
              </Button>
        ...
        <div className="container py-6">
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
            <div className="text-center">
              <p className="text-3xl font-bold text-red-600 tabular-nums">79.7%</p>
              <p className="text-xs text-muted-foreground mt-1">of Michigan races uncontested</p>
              <p className="text-[9px] text-muted-foreground/60">Ballotpedia 2024</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-foreground tabular-nums">~50,000</p>
              <p className="text-xs text-muted-foreground mt-1">governing board seats statewide</p>
              <p className="text-[9px] text-muted-foreground/60">Estimated across all board types</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-orange-600 tabular-nums">22%</p>
              <p className="text-xs text-muted-foreground mt-1">Detroit local election turnout</p>
              <p className="text-[9px] text-muted-foreground/60">Michigan SOS 2017</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold text-amber-600 tabular-nums">38%</p>
              <p className="text-xs text-muted-foreground mt-1">of officials say residents are engaged</p>
              <p className="text-[9px] text-muted-foreground/60">CLOSUP MPPS 2025 (down from 58%)</p>
            </div>
          </div>
        </div>
      </section>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {card.badge && <Badge variant="outline" className="text-[9px]">{card.badge}</Badge>}
                        </div>
          ...
          <CardContent>
            <div className="h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stateCompData} layout="vertical" margin={{ left: 110 }}>
                  ...
                  <ReferenceLine x={70} stroke="#6b7280" strokeDasharray="6 4" label={{ value: "National Avg 70%", position: "top", style: { fontSize: 10 } }} />
      ...
      {/* Regional Breakdown */}
      <section className="container pb-10">
        <h2 className="text-lg font-bold text-foreground mb-4">Regional Breakdown</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {MICHIGAN_RACE_DATA.map(region => (
            <Card key={region.region} className={`border ${getRegionBg(region.uncontestedPct)}`}>
              <CardContent className="py-4">
                <h3 className="text-sm font-bold text-foreground">{region.region}</h3>
                <p className="text-[10px] text-muted-foreground mb-3">{region.counties.slice(0, 4).join(", ")}{region.counties.length > 4 ? ` +${region.counties.length - 4}` : ""}</p>
                <div className="flex items-end gap-4 mb-3">
                  <div>
                    <p className={`text-3xl font-bold tabular-nums ${getRegionColor(region.uncontestedPct)}`}>
                      {region.uncontestedPct}%
                    </p>
                    <p className="text-[10px] text-muted-foreground">uncontested</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-foreground tabular-nums">{region.zeroCandidate}</p>
                    <p className="text-[10px] text-muted-foreground">zero candidates</p>
                  </div>
                </div>
                <div className="space-y-1 text-[10px]">
                  <p className="text-muted-foreground"><span className="font-medium text-foreground">Most uncontested:</span> {region.worstOfficeType}</p>
                  <p className="text-muted-foreground"><span className="font-medium text-foreground">Best opportunity:</span> {region.bestOpportunity}</p>
                </div>
                <p className="text-[9px] text-muted-foreground/60 mt-2">Ballotpedia / Michigan SOS {region.year}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section className="container pb-12">
        <h2 className="text-lg font-bold text-foreground mb-4">Why This Matters</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          <Card className="border-primary/20">
            <CardContent className="py-5">
              <Shield className="h-5 w-5 text-primary mb-2" />
              <h3 className="text-sm font-bold text-foreground mb-2">The 7% Problem</h3>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Only 7% of governing board members in the US found out about their seat through a public-facing channel. The rest were recruited through personal networks. accessmi.org changes this.
              </p>
              <p className="text-[9px] text-muted-foreground/60 mt-2">Source: Santa Cruz County 2025 Boards Survey</p>
            </CardContent>
          </Card>
