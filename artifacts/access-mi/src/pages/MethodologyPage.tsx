        ...
        <section>
          ...
          <div className="space-y-4 mb-6">
            {[
              ...
              {
                source: "USDA-FNA SNAP Retailer Locator",
                desc: "Currently authorized SNAP food retailers in Michigan, counted by county from the USDA-FNA (formerly FNS) Historical SNAP Retailer Locator bulk CSV (2005-2025 release, as of 2025-12). 'Currently authorized' = the row's End Date is blank. County rate = (retailer count / population) × 10,000, using PEP V2024 as the single named population vintage. No suppression: this is a count of authorized businesses, not a rare health event. Counties with zero retailers would render 0 rather than blank (none are zero in the current set). Point-to-county aggregation uses the source's own County field, normalized case-insensitively and with periods stripped; no coordinate-to-polygon geocoding was needed. The script asserts row conservation (assigned + dropped = MI currently-authorized total) and halts if more than 1% of rows would drop. Retailers are surfaced as a raw authorized count; the dataset's Store Type field is not interpreted as food-quality. The ArcGIS Hub FeatureServer linked from the USDA SNAP hub returns a Detroit-curated subset (~960 rows for STATE='MI') and is not used.",
                frequency: "Annual release; current-snapshot extract in app",
