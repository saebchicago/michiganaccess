                        ...
                        >
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <button
                                      ...
                                      aria-label={`Definition: ${metric.label}`}
                                    >
                          ...
                          {zips.map((z) => {
                            ...
                            return (
                              <td
                                key={z}
                                className="text-center px-3 py-3 tabular-nums text-xs text-foreground"
                              >
                                {display ? (
                                  <div className="flex flex-col items-center gap-0.5">
                                    <span>{display}</span>
...
export default function CompareZipsPage() {
  ...
  const handleCompare = useCallback(() => {
    ...
    if (nonMI.length > 0) {
      toast.error(
        `Access Michigan covers Michigan ZIP codes only. ${nonMI.join(", ")} ${nonMI.length === 1 ? "is" : "are"} outside Michigan.`,
      );
      return;
    }
    ...
    if (valid.length < 2) {
      toast.error(
        "Enter at least 2 valid Michigan ZIP codes (starting with 48 or 49).",
      );
      return;
    }
    setActiveZips(valid);
  ...
  const copyKeyStats = useCallback(() => {
    ...
    navigator.clipboard
      .writeText(text)
      .then(() => toast.success("Copied key stats with citations."));
  }, [summary, counties]);
  const copyMarkdown = useCallback(() => {
    ...
    for (const def of summary.metricDefinitions) {
      const vals = summary.zips.map((z) => {
        const mv = summary.values.find(
          (v) => v.metricId === def.id && v.zip === z,
        );
        return mv?.value != null
          ? (mv.displayValue ?? String(mv.value))
          : "n/a";
      });
        ...
        <div className="container max-w-3xl text-center">
          ...
          <motion.div initial="hidden" animate="visible" variants={fadeUp}>
            ...
            <p className="text-xs text-muted-foreground/70 mt-2 flex items-center justify-center gap-1.5">
              ...
              No made-up numbers. If we don't have data, we say so.
            </p>
          </motion.div>
        ...
        <div className="container max-w-5xl py-8 space-y-8 print:py-4">
          ...
          <div className="hidden print-header print:block text-center border-b border-border pb-3 mb-4">
            <p className="text-lg font-bold">
              Access Michigan - ZIP Comparison: {activeZips.join(" vs ")}
            </p>
            <p className="text-xs text-muted-foreground">
              Generated {new Date().toLocaleDateString()} · Data as of March
              2026 · accessmi.org/compare-zips
            </p>
          </div>
              ...
              <CardContent className="p-5">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex gap-2">
                    ...
                    ZIP codes are about systems, not the worth of people or
                    neighborhoods.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-warning mt-0.5 shrink-0">•</span>
                    Use these comparisons to understand where services,
                    infrastructure, and investment are stronger or weaker - not
                    to shame communities.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-warning mt-0.5 shrink-0">•</span>
                    Many patterns come from long histories of policy and
                    investment, not individual choices.
                  </li>
                </ul>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs">
                  ...
                  <span className="text-muted-foreground">
                    Coverage:{" "}
                    <Badge
                      variant={coverageBadgeVariant(summary.dataCoverage)}
                  ...
                  <span className="text-muted-foreground">
                    Sources: {summary.globalSources.join(", ")}
                  </span>
                  <Link
                    to="/methodology"
                    className="text-primary hover:underline flex items-center gap-0.5"
                  >
                    See full data &amp; methods{" "}
                    <ExternalLink className="h-3 w-3" />
                  </Link>
                </div>
                <p className="text-xs text-muted-foreground/70 mt-2">
                  Some metrics may not be available for every ZIP. Missing or
                  coarse data are clearly marked rather than estimated or filled
                  in.
                </p>
              </CardContent>
            </Card>
          ...
          {/* ═══ CHNA SUMMARY (when CHNA view active) ═══ */}
          {viewMode === "chna" && (
            <motion.div
              initial="hidden"
              whileInView="visible"
              ...
              <Card className="border-primary/20 bg-primary/5">
                <CardContent className="p-5 space-y-3">
                  ...
                  <ul className="text-xs text-muted-foreground space-y-1">
                    <li>
                      • Use these comparisons to identify priority geographies
                      and populations for your next CHNA implementation plan.
                    </li>
                    <li>
              ...
              <Card className="h-full">
                <CardContent className="p-5">
                  ...
                  <ul className="space-y-2.5 text-xs text-muted-foreground">
                    <li>
                      These ZIP codes have different levels of nearby primary
                      care and housing cost burden. Differences often reflect
                      where resources have been invested - not the quality of
                      the people who live there.
                    </li>
                    <li>
                      If your ZIP shows gaps in coverage or access, help may be
                      available through local programs and navigators.
                    </li>
            ...
            >
              <Card className="h-full">
                <CardContent className="p-5">
                  ...
                  <ul className="space-y-2.5 text-xs text-muted-foreground">
                    <li>
                      Use these metrics to identify where to deepen qualitative
                      work and community engagement for your next CHNA or
                      community benefit plan.
                    </li>
                    <li>
                      Use these numbers to validate lived experience, not
                      replace it - especially where data coverage is "Medium" or
                      "Limited."
                    </li>
                    <li>
                      For deeper analysis, export data and combine with your
                      internal claims, EHR, and survey data.
                    </li>
                  </ul>
                </CardContent>
              </Card>
            ...
            >
              <Card className="h-full">
                <CardContent className="p-5">
                  ...
                  <ul className="space-y-2.5 text-xs text-muted-foreground">
                    <li>
                      Download the data to build your own charts and analyses.
                    </li>
                    <li>
                      Always mention the data year and sources in your stories
                      or reports.
                    </li>
                    <li>
              ...
              <CardContent className="p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="gap-1.5 text-xs"
                    onClick={copyKeyStats}
                  >
                    <Copy className="h-3.5 w-3.5" /> Copy 3 key stats with
                    citations
                  </Button>
                  <Button
                    size="sm"
                ...
                <p className="text-[11px] text-muted-foreground mt-3">
                  CSV and JSON exports coming soon. Markdown and citation copy
                  work now with county-level data.
                </p>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <div className="flex justify-center">
            <AskCopilotButton
              ...
              label="Ask Access Michigan about this comparison"
            />
          </div>
            ...
            <Card className="border-primary/20 bg-primary/3">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-sm text-muted-foreground max-w-lg mx-auto mb-1">
                  If you're leading a CHNA, value-based care program, health
                  plan, utility, or local government project, we can help you
                  use this data responsibly.
                </p>
                <p className="text-xs text-muted-foreground/70 mb-5">
                  Access Michigan is an independent civic effort led by a
                  Michigan healthcare operations and data professional.
                </p>
                <div className="flex flex-wrap justify-center gap-3">
                  <Link to="/contact">
                    <Button className="gap-1.5">
                      <MessageSquare className="h-4 w-4" /> Talk with Saeb about
                      a project
                    </Button>
                  </Link>
                  <Link to="/methodology">
                    <Button variant="outline" className="gap-1.5">
                      <Shield className="h-4 w-4" /> Learn how we keep this
                      honest
                    </Button>
                  </Link>
                </div>
        ...
        <div className="container max-w-2xl py-16 text-center">
          ...
          >
            ...
            <p className="text-muted-foreground text-sm">
              Enter at least 2 Michigan ZIP codes above to see a side-by-side
              comparison.
            </p>
            <p className="text-xs text-muted-foreground/60 mt-2">
              Not sure which ZIPs to compare? Try your home ZIP and a
