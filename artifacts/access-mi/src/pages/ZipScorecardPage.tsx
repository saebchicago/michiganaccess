  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <p className="text-[10px] text-muted-foreground max-w-xs text-center">
        Composite of health outcomes, economic indicators, and environment
        factors.
      </p>
    </div>
  );
...
function MetricRow({
  ...
  label: string;
  value: number;
  stateAvg: number | null | undefined;
  unit: string;
  lowerIsBetter: boolean;
}) {
  // A missing or zero state baseline means we have no MI-level number to
  // compare against (PLACES does not publish a state average for every
  // measure). Surface that explicitly instead of computing diff = value - 0,
  // which would render the metric's own value as the delta.
  const hasBaseline = typeof stateAvg === "number" && stateAvg > 0;
  const diff = hasBaseline ? value - (stateAvg as number) : 0;
  const better = lowerIsBetter ? diff < 0 : diff > 0;

  return (
    <div className="flex items-center justify-between py-2 border-b border-border/40 last:border-0">
      <span className="text-sm text-foreground truncate max-w-[180px] sm:max-w-none">
        {label}
      </span>
      <div className="flex items-center gap-3">
        ...
        {hasBaseline ? (
          <span
            className={`text-[10px] font-semibold flex items-center gap-0.5 ${better ? "text-green-700" : "text-red-700"}`}
          >
            {better ? (
...
function computeEconomicScore(
  irsData: { avgAGI: number; eitcPct: number } | null,
  quickStats: { medianIncome: number } | null,
): number {
...
function computeEnvironmentScore(cdcDataMap: Record<string, number>): number {
  ...
  if (asthma != null) {
    const ratio =
      (MI_STATE_AVERAGES["Current Asthma"] - asthma) /
      MI_STATE_AVERAGES["Current Asthma"];
    score += Math.max(-20, Math.min(20, ratio * 40));
  }
  const physInact = cdcDataMap["Physical Inactivity"];
  if (physInact != null) {
    const ratio =
      (MI_STATE_AVERAGES["Physical Inactivity"] - physInact) /
      MI_STATE_AVERAGES["Physical Inactivity"];
    score += Math.max(-15, Math.min(15, ratio * 30));
  }
  const sleep = cdcDataMap["Short Sleep Duration"];
  if (sleep != null) {
    const ratio =
      (MI_STATE_AVERAGES["Short Sleep Duration"] - sleep) /
      MI_STATE_AVERAGES["Short Sleep Duration"];
    score += Math.max(-15, Math.min(15, ratio * 30));
  }
  return Math.round(Math.max(0, Math.min(100, score)));
}
...
function useZipMetrics(zip: string) {
  ...
  const healthResult = useMemo(() => {
    try {
      return computeHealthScore(cdcDataMap);
    } catch {
      return {
        score: 0,
        grade: "N/A",
  ...
  const econScore = useMemo(() => {
    try {
      return computeEconomicScore(irsData, quickStats);
    } catch {
      return 0;
    }
  }, [irsData, quickStats]);
  const envScore = useMemo(() => {
    try {
      return computeEnvironmentScore(cdcDataMap);
    } catch {
      return 0;
    }
  }, [cdcDataMap]);
...
export default function ZipScorecardPage() {
  ...
  const headerSubtext = [
    primary.city,
    primary.county ? `${primary.county} County` : "",
  ]
    .filter(Boolean)
    .join(", ");
  ...
  const handleCompare = useCallback(() => {
    if (
      compareInput.length === 5 &&
      /^\d{5}$/.test(compareInput) &&
      compareInput !== zip
    ) {
  ...
  const comparisonRows = useMemo(() => {
    ...
    const getUninsured = (cdcMap: Record<string, number>) =>
      cdcMap["No Health Insurance"] ?? null;
    const getDiabetes = (cdcMap: Record<string, number>) =>
      cdcMap["Diagnosed Diabetes"] ?? null;
    const getObesity = (cdcMap: Record<string, number>) =>
      cdcMap["Obesity"] ?? null;
    ...
    return [
      ...
      },
      {
        ...
        zip1: getUninsured(primary.cdcDataMap),
        zip2: getUninsured(comp.cdcDataMap),
        mi: MI_AVG_METRICS.uninsuredRate,
        unit: "%",
      },
      {
        label: "Diabetes",
        zip1: getDiabetes(primary.cdcDataMap),
        zip2: getDiabetes(comp.cdcDataMap),
        mi: MI_AVG_METRICS.diabetes,
        unit: "%",
      },
      {
        label: "Obesity",
        zip1: getObesity(primary.cdcDataMap),
        zip2: getObesity(comp.cdcDataMap),
        mi: MI_AVG_METRICS.obesity,
        unit: "%",
      },
      {
        label: "Avg AGI",
        zip1: primary.irsData?.avgAGI ?? null,
        zip2: comp.irsData?.avgAGI ?? null,
        mi: MI_AVG_METRICS.avgAGI,
        unit: "$",
      },
      {
        label: "EITC Rate",
        zip1: primary.irsData?.eitcPct ?? null,
        zip2: comp.irsData?.eitcPct ?? null,
        mi: MI_AVG_METRICS.eitcRate,
        unit: "%",
      },
      {
        label: "Avg Rent (2BR)",
        zip1: primary.fmrData?.fmr2br ?? null,
        zip2: comp.fmrData?.fmr2br ?? null,
        mi: MI_AVG_METRICS.avgRent,
        unit: "$",
      },
    ];
  ...
  const radarData = useMemo(() => {
    ...
    return [
      {
        metric: "Health",
        zip1: primary.healthResult.score,
        zip2: comp.healthResult.score,
        mi: 50,
      },
      {
        metric: "Economic",
        zip1: primary.econScore,
        zip2: comp.econScore,
        mi: 50,
      },
      {
        metric: "Environment",
        zip1: primary.envScore,
        zip2: comp.envScore,
        mi: 50,
      },
      {
    ...
    <Layout>
            ...
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              {primary.rurality && (
                <Badge variant="secondary" className="text-xs">
                  {RURALITY_ICONS[primary.rurality.class]}{" "}
                  {primary.rurality.class}
                </Badge>
              )}
            </div>
      ...
      <div className="container py-8 space-y-8">
              ...
              <CardContent className="py-4">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <Button
                    ...
                    disabled={compareInput.length !== 5 || compareInput === zip}
                  >
                    Compare
                  </Button>
                  {compareZip && (
            ...
            <Card className="border-michigan-gold/30 bg-michigan-gold/5">
              <CardContent className="py-8 text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground max-w-md mx-auto">
                  We don't have data for ZIP code <strong>{zip}</strong>. It may
                  not be a Michigan ZIP code, or it may be a PO Box /
          ...
          <Card className="border-destructive/30">
            <CardContent className="py-6 flex items-center gap-3">
              ...
              <p className="text-sm text-destructive">
                Failed to load CDC data. Local data sources are still shown
                below.
              </p>
            </CardContent>
          </Card>
            ...
            <Card className="border-border/60">
              <CardContent className="py-5 space-y-2">
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-block w-3 h-3 rounded-full ${scoreBand.color}`}
                  />
                  <h3 className="text-sm font-bold text-foreground">
                    {scoreBand.label}
                  </h3>
                  <span className="text-xs text-muted-foreground">
                    ({scoreBand.min}&#8211;{scoreBand.max})
                  </span>
                </div>
        ...
        {(() => {
          ...
          const disasterStatus = nri
            ? nri.compositeRisk > 30
              ? "Elevated"
            ...
            <Card className="border-michigan-teal/10 bg-gradient-to-r from-michigan-teal/[0.03] to-transparent">
              <CardContent className="py-4">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <div>
                    <p
                      className={`text-lg font-bold tabular-nums ${waterStatus === "Amber" ? "text-amber-600" : waterStatus === "Watch" ? "text-yellow-600" : "text-green-600"}`}
                    >
        ...
        <Tabs defaultValue="health" className="space-y-4">
              ...
              <Card>
                <CardContent className="py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    No CDC PLACES data available for ZIP {zip}. This ZCTA may
                    not have sufficient population for reporting.
                  </p>
                </CardContent>
              </Card>
            ) : (
              Object.entries(MEASURE_GROUPS)
                .slice(0, 3)
                .map(([group, measures]) => {
                  const available = measures.filter(
                    (m) => primary.cdcDataMap[m] != null,
                  );
                        ...
                        {available.map((m) => (
                          <MetricRow
                            ...
                            lowerIsBetter={
                              ...
                              m !== "High Blood Pressure Medication"
                            }
                          />
                        ))}
                      </CardContent>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {primary.quickStats && (
                <>
                  <EconCard
                    label="Population"
                    value={primary.quickStats.population.toLocaleString()}
                  ...
                  <EconCard
                    label="Median Rent"
                    value={primary.quickStats.medianRent}
                    format="$"
                    source="Census ACS 2022"
                  />
                  <EconCard
                    label="Property Tax (est.)"
                    value={primary.quickStats.propTaxEst}
                    format="$"
                    source="MI Treasury"
                  />
                  {primary.quickStats.cityTax !== "0%" && (
                    <EconCard
                      ...
                      value={primary.quickStats.cityTax}
                      source="MI Treasury"
                    />
                  )}
                  <EconCard
              ...
              {primary.irsData && (
                <>
                  <EconCard
                    label="Avg Adjusted Gross Income"
                    value={primary.irsData.avgAGI}
                    format="$"
                    benchmark={`MI avg: $${MI_IRS_STATE_AVERAGES.avgAGI.toLocaleString()}`}
                    source="IRS SOI 2021"
                  />
                  <EconCard
                    label="EITC Participation"
                    value={`${primary.irsData.eitcPct}%`}
                    benchmark={`MI avg: ${MI_IRS_STATE_AVERAGES.eitcPct}%`}
                    source="IRS SOI 2021"
                  />
                  <EconCard
                    label="Self-Employed"
                    value={`${primary.irsData.selfEmployedPct}%`}
                    benchmark={`MI avg: ${MI_IRS_STATE_AVERAGES.selfEmployedPct}%`}
                    source="IRS SOI 2021"
                  />
                </>
              )}
              {primary.fmrData && (
                <>
                  <EconCard
                    ...
                    value={primary.fmrData.fmr2br}
                    format="$"
                    benchmark={`MI avg: $${MI_FMR_AVERAGE_2BR}`}
                    source={HUD_FMR_SOURCE}
                  />
                  <EconCard
                    label="FMR Range (Studio–4BR)"
                    value={`$${primary.fmrData.fmr0br}–$${primary.fmrData.fmr4br}`}
                    source={HUD_FMR_SOURCE}
                  />
                </>
              )}
              {MICHIGAN_SAFMR[zip] && (
                <>
                  <EconCard
                    label="SAFMR (2BR)"
                    value={MICHIGAN_SAFMR[zip].safmr_2br}
                    format="$"
                    benchmark={`ZIP-level rent limit (FY${MICHIGAN_SAFMR[zip].fy})`}
                    source="HUD Small Area FMR"
                  />
                  <EconCard
                    label="SAFMR Range (Studio–4BR)"
                    value={`$${MICHIGAN_SAFMR[zip].safmr_0br}–$${MICHIGAN_SAFMR[zip].safmr_4br}`}
                    source="HUD Small Area FMR"
                  />
                </>
              )}
          ...
          <TabsContent value="resources" className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {(MEASURE_GROUPS["Social Needs"] ?? []).filter(
                (m) => primary.cdcDataMap[m] != null,
              ).length > 0 && (
                <Card className="sm:col-span-2 lg:col-span-3">
                  <CardContent className="py-4">
                    <h3 className="text-sm font-bold text-foreground mb-2">
                      Social Needs (CDC PLACES)
                    </h3>
                    {MEASURE_GROUPS["Social Needs"]
                      .filter((m) => primary.cdcDataMap[m] != null)
                      .map((m) => (
                        <MetricRow
                          key={m}
                          label={m}
                  ...
                  return (
                    <Card className="sm:col-span-2 lg:col-span-3">
                      <CardContent className="py-4">
                        ...
                        <p className="text-[9px] text-muted-foreground mt-2">
                          Source: EPA EJSCREEN v2.3 ({ej.data_year}). Higher
                          percentiles = greater environmental burden.
                        </p>
                      </CardContent>
                    </Card>
                  );
              ...
              {[
                {
                  ...
                  desc: `Search providers near ${zip}`,
                  href: `/find-care?zip=${zip}`,
                  icon: Stethoscope,
                },
                {
                  title: "Community Resources",
                  desc: primary.county
                    ? `Food, housing, transport in ${primary.county} County`
                    : "Food, housing, transport & more",
                  href: primary.county
                    ? `/resources?county=${primary.county}`
                    : "/resources",
                  icon: Heart,
                },
                {
                  title: "Financial Assistance",
                  desc: "Medicaid, SNAP, LIHEAP & more",
                  href: "/financial-help",
                  icon: DollarSign,
                },
              ].map((r) => (
            ...
            {demographics ? (
              <div className="space-y-4">
                <Card>
                  <CardContent className="py-4">
                    ...
                    <p className="text-2xl font-bold text-foreground tabular-nums">
                      {demographics.totalPop.toLocaleString()}
                    </p>
                    <p className="text-[9px] text-muted-foreground">
                      {demographics.source}
                    </p>
                ...
                <Card>
                  <CardContent className="py-4">
                    ...
                    <div className="h-6 rounded-full overflow-hidden flex mb-3">
                      <div
                        className="h-full bg-[#4f86c6]"
                        style={{ width: `${demographics.white_alone_pct}%` }}
                        title={`White ${demographics.white_alone_pct.toFixed(1)}%`}
                      />
                      <div
                        className="h-full bg-[#f59e0b]"
                        style={{ width: `${demographics.black_alone_pct}%` }}
                        title={`Black ${demographics.black_alone_pct.toFixed(1)}%`}
                      />
                      <div
                        className="h-full bg-[#10b981]"
                        style={{ width: `${demographics.hispanic_pct}%` }}
                        title={`Hispanic ${demographics.hispanic_pct.toFixed(1)}%`}
                      />
                      <div
                        className="h-full bg-[#8b5cf6]"
                        style={{ width: `${demographics.asian_alone_pct}%` }}
                        title={`Asian ${demographics.asian_alone_pct.toFixed(1)}%`}
                      />
                      <div
                        className="h-full bg-[#94a3b8]"
                        style={{ width: `${demographics.multiracial_pct}%` }}
                        title={`Other ${demographics.multiracial_pct.toFixed(1)}%`}
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 text-xs">
                      <span>
                        <span className="inline-block h-2 w-2 rounded-full bg-[#4f86c6] mr-1" />
                      ...
                      <span>
                        ...
                        Other {demographics.multiracial_pct.toFixed(1)}%
                      </span>
                    </div>
                    <p className="text-[9px] text-muted-foreground mt-2">
                      {demographics.source}
                  ...
                  <Card>
                    <CardContent className="py-4">
                      ...
                      <EconCard
                        ...
                        value={`${demographics.lep_pct.toFixed(1)}%`}
                        benchmark={`MI avg: ${demographics.miStateLEPPct}%`}
                        source={demographics.source}
                      />
                      {demographics.spanish_speakers_pct > 1 && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Spanish speakers:{" "}
                          {demographics.spanish_speakers_pct.toFixed(1)}%
                        </p>
                      )}
                      {demographics.arabic_speakers_pct > 1 && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Arabic speakers:{" "}
                          {demographics.arabic_speakers_pct.toFixed(1)}%
                        </p>
                      )}
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="py-4">
                      ...
                      <EconCard
                        ...
                        value={`${demographics.disability_pct.toFixed(1)}%`}
                        benchmark={`MI avg: ${demographics.miStateDisabilityPct}%`}
                        source={demographics.source}
                      />
                      <p className="text-xs text-muted-foreground mt-2">
                        Renter: {demographics.renter_pct.toFixed(1)}% · Owner:{" "}
                        {demographics.owner_pct.toFixed(1)}%
                      </p>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card className="border-amber-200/50 dark:border-amber-900/30 bg-amber-50/30 dark:bg-amber-950/10">
                  <CardContent className="py-4">
                    ...
                    <p className="text-[9px] text-muted-foreground/60 mt-1">
                      Framing: CDC Health Equity / RWJF research
                    </p>
                  </CardContent>
                </Card>
                ...
                <Suspense fallback={null}>
                  <ZIPNarratives
                    ...
                    equityScore={primary.compositeScore}
                    equityTier={compositeTier.tier}
                    topHealthConcern={
                      primary.healthResult.concerns[0] ?? "Multiple factors"
                    }
                    medianIncome={demographics.medianHouseholdIncome}
                    renterPct={demographics.renter_pct}
                    lepPct={demographics.lep_pct}
                    disabilityPct={demographics.disability_pct}
                  />
                </Suspense>
                ...
                <div className="flex justify-end">
                  <Suspense fallback={null}>
                    <CHNAExport
                      ...
                      equityTier={compositeTier.tier}
                      topConcerns={[
                        ...primary.healthResult.concerns,
                        ...(primary.healthResult.strengths.length === 0
                          ? ["Access barriers"]
                          : []),
                      ]}
                    />
              ...
              <Card>
                <CardContent className="py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Loading Census demographics for ZIP {zip}...
                  </p>
                  <p className="text-[9px] text-muted-foreground/60 mt-1">
          ...
          <TabsContent value="access" className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    />
                    <EconCard
                      label="Gigabit Coverage"
                      value={`${bb.pct_gigabit_covered}%`}
                      source="FCC Broadband Map 2024"
                    />
                    <EconCard
                      label="Unserved Locations"
                      value={bb.unserved_locations.toLocaleString()}
                      benchmark="< 25/3 Mbps"
                      source="FCC Broadband Map 2024"
                    />
                  </>
              ...
              {(() => {
                ...
                return (
                  <>
                    <EconCard
                      label="Food Access"
                      value={fa.classification}
                      source="USDA FARA 2019"
                    />
                    <EconCard
                      label="Low-Access Tracts"
                      value={`${fa.lowAccessPct.toFixed(1)}%`}
                      benchmark={`${fa.lowAccessTracts} of ${fa.totalTracts} tracts`}
                      source="USDA FARA 2019"
                    />
                    <EconCard
                      label="No Vehicle + Low Access"
                      value={fa.noVehicleLowAccessPop.toLocaleString()}
                      benchmark="Most vulnerable population"
                      source="USDA FARA 2019"
                    />
                  </>
                );
              ...
              {!getBroadbandByCounty(primary.county ?? "") &&
                !getFoodAccessByCounty(primary.county ?? "") && (
                  <Card className="sm:col-span-2 lg:col-span-3">
                    <CardContent className="py-8 text-center">
                      <Info className="h-8 w-8 text-muted-foreground mx-auto mb-3" />
                      <p className="text-sm text-muted-foreground">
                        No infrastructure data seeded for{" "}
                        {primary.county ?? "this"} county yet.
                      </p>
                    </CardContent>
                  </Card>
                )}
            ...
            <Card className="bg-primary/[0.03] border-primary/20">
              <CardContent className="py-4 text-center">
                <p className="text-sm text-muted-foreground mb-2">
                  See all data layers on one map
                </p>
                <Link
                  to={`/map/layers?zip=${zip}`}
