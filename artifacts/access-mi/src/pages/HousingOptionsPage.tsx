...
const situationButtons: {
  ...
  {
    value: "emergency",
    label: "I need emergency shelter or I'm unsafe tonight",
    desc: "Immediate crisis housing, shelters, and safety resources",
    icon: AlertTriangle,
  },
  {
    value: "at_risk",
    label: "I'm at risk of losing housing soon",
    desc: "Eviction prevention, emergency rental assistance, legal help",
    icon: Shield,
  },
  {
    value: "affordable",
    label: "I'm looking for affordable rentals or subsidized housing",
    desc: "Waitlists, vouchers, housing authorities, and listings",
    icon: Building2,
  },
];
  ...
  return (
    ...
    >
      ...
      <span className="sr-only">
        (opens external site - you are leaving Access Michigan)
      </span>
    </a>
  );
}
...
export default function HousingOptionsPage() {
  ...
  const defaultZip =
    paramZip || profile.primaryZip || granularLocation.zip || "";

  const [step, setStep] = useState(1);
  const [zipInput, setZipInput] = useState(defaultZip);
  const [selectedCounty, setSelectedCounty] = useState<MichiganCounty | null>(
    county,
  );
  const [situation, setSituation] = useState<Situation | null>(
    profile.housingStatus === "homeless"
      ? "emergency"
      : profile.housingStatus === "at_risk"
        ? "at_risk"
        : null,
  );
  const [income, setIncome] = useState<IncomeBand>(
    profile.incomeBand ?? "unknown",
  );
  ...
  const { data: housingResources } = useCommunityResources(
    "housing",
    selectedCounty,
  );
  const { data: shelterResources } = useCommunityResources(
    "shelter",
    selectedCounty,
  );
      ...
      <div className="container max-w-3xl py-6 space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            A few questions. Real resources. Nothing stored.
          </p>
        </div>
              ...
              <Card>
                <CardContent className="pt-5 space-y-4">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <MapPin className="h-4 w-4 text-primary" /> Step 1: Where
                    are you located?
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="hz" className="text-xs">
                      ZIP code
                    </Label>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setSelectedCounty(v as MichiganCounty)
                    }
                  >
                    <SelectTrigger>
                  ...
                  <Button
                    ...
                    disabled={!selectedCounty && zipInput.length < 5}
                    onClick={() => {
                      resolveZip();
                      setStep(2);
                    }}
          ...
          {step === 2 && (
            <motion.div key="s2" {...fadeSlide} className="space-y-4">
              <Card>
                <CardContent className="pt-5 space-y-4">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <Home className="h-4 w-4 text-primary" /> Step 2: What best
          ...
          {step === 3 && (
            <motion.div key="s3" {...fadeSlide} className="space-y-4">
              <Card>
                <CardContent className="pt-5 space-y-4">
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <DollarSign className="h-4 w-4 text-primary" /> Step 3:
                    Roughly where is your income?
                  </div>
                  <p className="text-xs text-muted-foreground">
                    This helps us suggest the right programs. We don't store
                    this.
                  </p>
          ...
          {step === 4 && (
            <motion.div key="s4" {...fadeSlide} className="space-y-5">
                ...
                <Card className="border-destructive/50 bg-destructive/5">
                  <CardContent className="pt-4 space-y-2">
                    <div className="flex items-center gap-2 text-destructive font-semibold text-sm">
                      <Phone className="h-4 w-4" /> If you are in immediate
                      danger, call 911
                    </div>
                    <div className="text-xs space-y-1 text-foreground/80">
                      <p>
                        <strong>Michigan 211:</strong>{" "}
                        <a href="tel:211" className="text-primary underline">
                          Call 211
                        </a>{" "}
                        - shelter, food, and crisis services 24/7
                      </p>
                      <p>
                        ...
                        - safety planning, shelter referrals
                      </p>
                      <p>
                        <strong>988 Suicide & Crisis Lifeline:</strong>{" "}
                        <a href="tel:988" className="text-primary underline">
                          Call or text 988
                        </a>
                      </p>
                    </div>
                ...
                <CardContent className="pt-5 space-y-3">
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span>
                        <ExtLink href="https://mi211.org">
                          Michigan 211 - Housing & Shelter
                        </ExtLink>{" "}
                        - search for emergency shelters and crisis housing in{" "}
                        {countyLabel}.
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                      <span>
                        <ExtLink href="https://www.michigan.gov/mdhhs/assistance-programs/emergency-relief">
                          MDHHS Emergency Relief
                        </ExtLink>{" "}
                        - state emergency assistance for rent, utilities, and
                        home repairs.
                      </span>
                    </li>
                    ...
                    <li className="flex items-start gap-2">
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                      <span>
                        <ExtLink href="https://www.salvationarmyusa.org/usn/provide-shelter/">
              ...
              <Card>
                <CardContent className="pt-5 space-y-3">
                  <div className="flex items-center gap-2 font-semibold text-sm">
                    <Building2 className="h-4 w-4 text-primary" /> Subsidized &
                    Affordable Housing
                  </div>
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                      <span>
                        <ExtLink href="https://resources.hud.gov/">
                          HUD Resource Locator
                        </ExtLink>{" "}
                        - search for subsidized apartments, public housing, and
                        voucher programs.
                      </span>
                    </li>
                    <li className="flex items-start gap-2">
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                      <span>
                        <ExtLink href="https://www.michigan.gov/mshda">
                          Michigan State Housing Development Authority (MSHDA)
                        </ExtLink>{" "}
                        - rental assistance, homeownership programs, and
                    ...
                    <li className="flex items-start gap-2">
                      <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                      <span>
                        <ExtLink href="https://affordablehousingonline.com/housing-search/Michigan">
                          Affordable Housing Online - Michigan
                        </ExtLink>{" "}
                        - search waitlists and income-restricted listings.
                      </span>
                    </li>
                  ...
                  <div className="rounded-md bg-muted/40 p-3 text-xs text-muted-foreground space-y-1 mt-2">
                    <p>
                      <strong>Waitlists:</strong> Many subsidized programs have
                      waitlists. Apply to multiple programs at once to improve
                      your chances.
                    </p>
                    <p>
                      <strong>Documentation:</strong> You'll typically need
                      proof of identity, income, and household size. Call ahead
                      to confirm.
                    </p>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="pt-5 space-y-3">
                  <div className="flex items-center gap-2 font-semibold text-sm">
                    <Search className="h-4 w-4 text-primary" /> Market Rentals &
                    Safer Searching
                  </div>
                  <div className="text-sm text-muted-foreground space-y-2">
                    <p>
                      When searching for market-rate rentals, keep these tips in
                      mind:
                    </p>
                    <ul className="space-y-1 list-disc pl-4">
                      <li>
                        Use multiple listing sites to compare prices and
                        availability.
                      </li>
                      <li>
                        Avoid cash-only arrangements and unofficial leases -
                        always get things in writing.
                      </li>
                      <li>
                        Michigan fair housing law protects against
                        discrimination based on race, religion, sex, disability,
                        familial status, and more. If you suspect
                        discrimination, contact the{" "}
                        <ExtLink href="https://www.michigan.gov/mdcr">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    <Badge variant="outline" className="text-xs">
                      <ExtLink href="https://www.apartments.com">
                        Apartments.com
                      </ExtLink>
                    </Badge>
                    <Badge variant="outline" className="text-xs">
                      <ExtLink href="https://www.zillow.com/homes/for_rent/">
                        Zillow Rentals
                      </ExtLink>
                    </Badge>
                    <Badge variant="outline" className="text-xs">
                      <ExtLink href="https://www.realtor.com/apartments/">
                        Realtor.com Rentals
                      </ExtLink>
                    </Badge>
                  </div>
                  <p className="text-[10px] text-muted-foreground/60 mt-1">
                    These are common search sites, not endorsements by Access
                    Michigan.
                  </p>
                </CardContent>
              </Card>
              ...
              {(situation === "at_risk" || situation === "emergency") && (
                <Card>
                  <CardContent className="pt-5 space-y-3">
                    <div className="flex items-center gap-2 font-semibold text-sm">
                      <Scale className="h-4 w-4 text-primary" /> Legal Help
                    </div>
                    <ul className="space-y-2 text-sm">
                      <li className="flex items-start gap-2">
                        <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                        <span>
                          <ExtLink href="https://michiganlegalhelp.org/self-help-tools/housing">
                            Michigan Legal Help - Housing
                          </ExtLink>{" "}
                          - free legal information about eviction, landlord
                          disputes, and housing rights.
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <ChevronRight className="h-3.5 w-3.5 mt-0.5 shrink-0 text-muted-foreground" />
                        <span>
                          <ExtLink href="https://www.hud.gov/findacounselor">
                            HUD Housing Counseling
                          </ExtLink>{" "}
                          - free, HUD-approved housing counselors for
                          foreclosure prevention and rental help.
                        </span>
                      </li>
                    </ul>
                  </CardContent>
        ...
        <div className="rounded-md bg-muted/30 border border-border p-3 flex items-start gap-2">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Access Michigan connects you with public resources and does not
            determine eligibility for any program. We do not store any
            information you enter on this page. External links are clearly
            marked - when you click them, you leave Access Michigan.
          </p>
        </div>
      </div>
    </Layout>
