...
import {
 ...
 describe("Community Opportunity Atlas data contracts", () => {
  ...
  it("resolves ZIPs without pretending county metrics are ZIP metrics", () => {
    ...
    expect(place?.resolutionNote).toContain("not represented as ZIP-level measurements");
  });
  ...
  it("keeps raw source provenance separate from modeled comparison math", () => {
    ...
    expect(insights.some((item) => item.provenanceLabel === "VERIFIED")).toBe(true);
    expect(insights.every((item) => item.benchmark.provenanceLabel === "MODELED")).toBe(true);
    expect(insights.every((item) => item.nativeResolution === "county")).toBe(true);
  });

  it("does not silently upgrade fine-grain lenses before ingestion or permission review", () => {
    const statuses = Object.fromEntries(OPPORTUNITY_LENSES.map((lens) => [lens.id, lens.status]));
    ...
    expect(OPPORTUNITY_LENSES.every((lens) => lens.provenanceLabel === "PENDING")).toBe(true);
  });
