...
export default function MichiganEnvBurdenMap({
  initialCounty = "26163",
}: Props) {
          ...
          .on("click", (_event: MouseEvent, f: any) => {
            ...
            svg
              ...
              .attr("stroke-width", (d: any) =>
                String(d.id).padStart(5, "0") === fips ? 2.5 : 0.5,
              )
              .attr("opacity", (d: any) =>
                String(d.id).padStart(5, "0") === fips ? 1 : 0.85,
              );
          });
  ...
  return (
    <div className="grid gap-4" style={{ gridTemplateColumns: "1fr" }}>
      ...
      `}</style>
      <div
        className="env-burden-grid grid gap-4"
        ...
        >
                ...
                <>
                  <p className="text-muted-foreground">
                    ...
                    <span className="text-foreground font-medium">
                      {countyEJData[tooltip.fips].ej_index}th pct
                    </span>
                  </p>
                  <p className="text-muted-foreground">
                    PM2.5:{" "}
                    <span className="text-foreground font-medium">
                      {countyEJData[tooltip.fips].pm25_percentile}th pct
                    </span>
                  </p>
                  <p className="text-muted-foreground">
                    Ozone:{" "}
                    <span className="text-foreground font-medium">
                      {countyEJData[tooltip.fips].ozone_percentile}th pct
                    </span>
                  </p>
                </>
              ) : (
                <p className="text-muted-foreground italic">
                  No sub-county EJScreen data
                </p>
              )}
            </div>
          )}
          ...
          <div className="absolute bottom-2 left-2 flex items-center gap-2 bg-background/80 rounded px-2 py-1 text-xs text-muted-foreground flex-wrap">
            ...
            <div
              ...
              style={{
                background: "linear-gradient(to right, #e8f4f0, #0A4C95)",
              }}
            />
            ...
            {ejData ? (
                ...
                <div className="mt-3 flex gap-4 text-xs text-muted-foreground border-t border-border/50 pt-3">
                  <span>
                    ...
                    <span className="font-medium text-foreground">
                      {ejData.pct_low_income.toFixed(1)}%
                    </span>
                  </span>
                  <span>
                    Minority:{" "}
                    <span className="font-medium text-foreground">
                      {ejData.pct_minority.toFixed(1)}%
                    </span>
                  </span>
                </div>
                <p className="mt-1.5 text-[10px] text-muted-foreground">
                  ZCTA {ejData.zcta} (highest-burden ZCTA in county). National
                  percentile 0-100, higher = more burdened. Data year:{" "}
                  {ejData.data_year}.
                </p>
              </>
            ) : (
              <div className="py-6 text-center">
                <p className="text-sm text-muted-foreground">
                  Sub-county EJScreen data not yet available for {countyName}{" "}
                  County.
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Statewide data integration in progress.
                </p>
              </div>
            )}
          </div>
