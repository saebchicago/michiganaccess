...
const FUNNEL_CURRENT = [
  ...
  {
    stage: "Screened for SDOH",
    value: 478440,
    pct: 27.0,
    source: "Trinity Health reference rate (27.4%)",
  },
  {
    stage: "Needs Identified",
    value: 129178,
    pct: 27.0,
    source: "National ~27% positive screen (NACHC)",
  },
  {
    stage: "Referred to Services",
    value: 64589,
    pct: 50.0,
    source: "National avg ~50% referral (AHC model)",
  },
  {
    stage: "Referral Completed",
    value: 7105,
    pct: 11.0,
    source: "National ~11% resolution (HealthAffairs)",
  },
  {
    stage: "Health Improvement Documented",
    value: null,
    pct: null,
    source: "No data available yet",
  },
];
...
const FUNNEL_WITH_AM = [
  ...
  },
  {
    ...
    value: 708800,
    pct: 40.0,
    source: "Projected: unified screening via Access Michigan",
  },
  {
    stage: "Needs Identified",
    value: 191376,
    pct: 27.0,
    source: "Same positive rate, larger screened pool",
  },
  {
    stage: "Referred to Services",
    value: 114826,
    pct: 60.0,
    source: "Projected: improved referral w/ CIE integration",
  },
  {
    stage: "Referral Completed",
    value: 39041,
    pct: 34.0,
    source: "Projected: 11% → 34% w/ closed-loop tracking",
  },
  {
    stage: "Health Improvement Documented",
    value: null,
    pct: null,
    source: "Future measurement framework",
  },
];
...
export default function DetectionGapFunnel({
  variant = "full",
}: DetectionGapFunnelProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge
              variant="outline"
              className="text-[9px] font-normal uppercase tracking-wider text-muted-foreground border-border"
            >
              Modeled Estimate
            </Badge>
          </div>
          <p className="text-xs text-muted-foreground">
            From 1.77M beneficiaries to documented outcomes - where the system
            loses people
          </p>
          <p className="text-[10px] text-muted-foreground mt-1">
            Method: Michigan MCO enrollment (1.77M) × national benchmark rates
            (NACHC, AHC Model). Not measured MI outcomes.{" "}
            <Link
              to="/methodology#sdoh-funnel"
      ...
      <div className="space-y-2">
        {data.map((stage, i) => {
          const barWidth = stage.value
            ? Math.max(8, (stage.value / maxVal) * 100)
            : 5;
          const dropoff =
            i > 0 && data[i - 1].value && stage.value
      ...
      <p className="text-[9px] text-muted-foreground/60">
        Sources: HMA March 2025 (MCO enrollment), Trinity Health FY2025
        (screening reference), NACHC (positive rate), AHC Model (referral rate),
        Health Affairs 2023 (resolution rate).
        {mode === "projected" &&
          " Projected improvements are platform targets, not achieved outcomes."}
      </p>
    </div>
  );
}
