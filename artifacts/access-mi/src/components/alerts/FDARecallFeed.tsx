...
export default function FDARecallFeed() {
  ...
  if (isLoading) {
    return (
      <Card><CardContent className="py-6 flex items-center justify-center gap-2">
        <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />
        <span className="text-xs text-muted-foreground">Checking FDA recalls...</span>
      </CardContent></Card>
    );
  }
  ...
  return (
    <Card className={hasRecalls ? "border-amber-500/20" : "border-michigan-forest/20"}>
      ...
      <CardContent className="space-y-2">
        {hasRecalls ? (
          recalls!.map((r) => (
            <div key={r.recallNumber} className="rounded-lg border border-border p-3 space-y-1">
              <div className="flex items-start justify-between gap-2">
                <p className="text-xs text-foreground line-clamp-2">{r.productDescription}</p>
                <Badge className={`text-[8px] shrink-0 ${CLASS_COLORS[r.classification] || "bg-gray-400 text-white"}`}>
                  {r.classification}
                </Badge>
              </div>
              <p className="text-[10px] text-muted-foreground">{r.reason}</p>
              <div className="flex justify-between text-[10px] text-muted-foreground">
                <span>{r.recallingFirm}{r.city ? `, ${r.city}` : ""}</span>
