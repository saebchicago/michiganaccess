...
function generateInsights(
  data: PlacesMeasure[],
  zip: string,
): { text: string; type: "warning" | "good" | "info" }[] {
  ...
  for (const d of data) {
    ...
    if (gap > worstGap.gap)
      worstGap = { measure: d.short_question_text, gap, value: d.data_value };
    if (gap < bestGap.gap)
      bestGap = {
        measure: d.short_question_text,
        gap,
        value: d.data_value,
        stateAvg,
      };
  ...
  if (worstGap.gap > 2)
    insights.push({
      text: `${zip} has ${worstGap.measure} at ${worstGap.value.toFixed(1)}% - ${worstGap.gap.toFixed(1)} points above the Michigan average.`,
      type: "warning",
    });
  if (bestGap.gap < -2)
    insights.push({
      text: `${zip} performs better than Michigan on ${bestGap.measure} (${bestGap.value.toFixed(1)}% vs ${bestGap.stateAvg.toFixed(1)}%).`,
      type: "good",
    });

  const uninsured = data.find(
    (d) => d.short_question_text === "Lack of Health Insurance",
  );
  if (uninsured && uninsured.data_value > 10)
    insights.push({
      text: `${uninsured.data_value.toFixed(1)}% lack health insurance in ${zip}. See Michigan coverage program rules.`,
      type: "info",
    });
...
export default function ZipIntelligenceBuilder({
  initialZip,
  initialMeasures,
}: {
  initialZip?: string;
  ...
  const [selected, setSelected] = useState<Set<string>>(
    new Set(initialMeasures || POPULAR_MEASURES),
  );
  ...
  const chartData = useMemo(() => {
    return filteredData.map((d) => {
      const entry: Record<string, string | number> = {
        name:
          d.short_question_text.length > 20
            ? d.short_question_text.substring(0, 18) + "…"
            : d.short_question_text,
      ...
      if (activeZip2 && zip2Data) {
        const z2 = zip2Data.find(
          (z) => z.short_question_text === d.short_question_text,
        );
        ...
        <CardContent className="p-5">
          <form onSubmit={handleSearch} className="flex gap-2 items-center">
            ...
            <Input
              ...
              placeholder="Enter Michigan ZIP code"
              className="max-w-[180px] font-mono text-center"
            />
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {zipData.length} measures available
                </p>
              </div>
              {!showZip2 && (
                <Button
                ...
                <CardContent className="p-3 flex gap-2 items-center">
                  ...
                  >
                    <Input
                      ...
                      value={zip2Input}
                      onChange={(e) =>
                        setZip2Input(e.target.value.replace(/\D/g, ""))
                      }
                      placeholder="Compare ZIP"
                      className="max-w-[140px] font-mono text-center text-xs"
                    />
                    <Button
                      type="submit"
                      size="sm"
                      variant="outline"
                      disabled={zip2Input.length !== 5}
                    >
                      Go
                    </Button>
                  </form>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">
                  Select measures to chart ({selected.size}/12)
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex flex-wrap gap-1.5">
                  <Button
                    ...
                    onClick={() => setSelected(new Set(POPULAR_MEASURES))}
                  >
                ...
                {Object.entries(MEASURE_GROUPS).map(([category, measures]) => (
                  <details key={category} className="group">
                    <summary className="text-xs font-medium text-foreground cursor-pointer hover:text-primary list-none flex items-center gap-1">
                      <svg
                        className="h-3 w-3 text-muted-foreground transition-transform group-open:rotate-90"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                    ...
                    <div className="flex flex-wrap gap-1 mt-1 ml-4">
                      {measures.map((m) => {
                        const hasData = zipData.some(
                          (d) => d.short_question_text === m,
                        );
                        return (
                          <button
                            key={m}
              ...
              <Card>
                ...
                <CardContent>
                  {view === "bar" && (
                    <ResponsiveContainer
                      width="100%"
                      height={Math.max(filteredData.length * 60, 300)}
                    >
                      <BarChart
                        data={chartData}
                        layout="vertical"
                      ...
                      <table className="w-full text-sm min-w-[400px]">
                        <thead>
                          <tr className="border-b border-border text-left">
                            ...
                            <th className="pb-2 text-xs font-medium text-right">
                              {activeZip}
                            </th>
                            {activeZip2 && (
                              <th className="pb-2 text-xs font-medium text-right">
                        ...
                        <tbody>
                          {filteredData.map((d) => {
                            const stateAvg =
                              MI_STATE_AVERAGES[d.short_question_text] || 0;
                            const diff = d.data_value - stateAvg;
                            const z2 = zip2Data?.find(
                              (z) =>
                                z.short_question_text === d.short_question_text,
                            );
                            return (
                              <tr
                                key={d.short_question_text}
                              ...
                              >
                                ...
                                <td className="py-2 text-xs text-right text-muted-foreground">
                                  {(
                                    US_AVERAGES[d.short_question_text] || 0
                                  ).toFixed(1)}
                                  %
                                </td>
                                <td
                  ...
                  {view === "trend" && (
                    <div className="space-y-2">
                      {!trendData ||
                      Object.keys(trendData.year2023).length === 0 ? (
                        <p className="text-sm text-muted-foreground text-center py-8">
                          Loading trend data or no historical data available for
                          this ZIP.
                        </p>
                      ) : (
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm min-w-[400px]">
                            ...
                            <tbody>
                              {filteredData.map((d) => {
                                const prev =
                                  trendData.year2023[d.short_question_text];
                  ...
                  <div
                    ...
                    className={`rounded-lg border p-3 flex items-start gap-2 ${
                      insight.type === "warning"
                        ? "border-michigan-coral/20 bg-michigan-coral/5"
                        : insight.type === "good"
                          ? "border-michigan-forest/20 bg-michigan-forest/5"
