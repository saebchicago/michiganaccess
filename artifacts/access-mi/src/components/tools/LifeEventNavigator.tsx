...
interface Resource { name: string; type: string; href: string; }
interface LifeEvent { id: string; icon: typeof Baby; label: string; resources: Resource[]; }

const EVENTS: LifeEvent[] = [
  {
    id: "baby", icon: Baby, label: "Just had a baby",
    resources: [
      { name: "WIC (Women, Infants & Children)", type: "nutrition", href: "/financial-help" },
      { name: "Medicaid for newborns", type: "insurance", href: "/find-care" },
      { name: "Maternal health data for your county", type: "data", href: "/maternal-health" },
      { name: "Postpartum depression resources", type: "health", href: "/resources" },
    ],
  },
  {
    id: "lost-job", icon: Briefcase, label: "Lost my job",
    resources: [
      { name: "Michigan unemployment benefits (UIA)", type: "financial", href: "/financial-help" },
      { name: "Healthy Michigan Plan (Medicaid)", type: "insurance", href: "/find-care" },
      { name: "SNAP food assistance", type: "food", href: "/resources" },
      { name: "LIHEAP energy assistance", type: "energy", href: "/environment#programs" },
    ],
  },
  {
    id: "turning-65", icon: Cake, label: "Turning 65",
    resources: [
      { name: "Medicare enrollment guide", type: "insurance", href: "/find-care" },
      { name: "Michigan MMAP counseling (free)", type: "counseling", href: "/resources" },
      { name: "Prescription drug assistance", type: "pharmacy", href: "/financial-help" },
      { name: "Senior food programs", type: "food", href: "/resources" },
    ],
  },
  {
    id: "housing", icon: Home, label: "Facing eviction",
    resources: [
      { name: "Michigan Legal Help (free)", type: "legal", href: "/resources" },
      { name: "Emergency rental assistance", type: "financial", href: "/financial-help" },
      { name: "2-1-1 immediate help", type: "crisis", href: "/resources" },
      { name: "Utility shutoff protections", type: "energy", href: "/environment#programs" },
    ],
  },
  {
    id: "diagnosis", icon: Stethoscope, label: "New diagnosis",
    resources: [
      { name: "Find specialists near you", type: "care", href: "/find-care" },
      { name: "Clinical trials in Michigan", type: "research", href: "/clinical-trials" },
      { name: "Insurance appeal tools", type: "insurance", href: "/health/insurance-appeals" },
      { name: "Support groups", type: "support", href: "/resources" },
    ],
  },
  {
    id: "new-to-mi", icon: Globe, label: "New to Michigan",
    resources: [
      { name: "Community health centers (FQHCs)", type: "care", href: "/find-care" },
      ...
      <div className="container max-w-4xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {EVENTS.map((event) => (
            ...
            >
              <event.icon className={`h-7 w-7 mx-auto mb-2 ${active === event.id ? "text-primary" : "text-muted-foreground"}`} />
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-sm flex items-center gap-2">
                      <selected.icon className="h-4 w-4 text-primary" /> {selected.label}
                    </CardTitle>
                    <button onClick={() => setActive(null)} className="text-muted-foreground hover:text-foreground p-1">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {selected.resources.map((r) => (
                      ...
                      >
                        <Badge className={`text-[8px] shrink-0 ${TYPE_COLORS[r.type] || "bg-muted text-muted-foreground"}`}>
                          {r.type}
                        </Badge>
