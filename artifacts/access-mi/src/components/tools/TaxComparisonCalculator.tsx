...
export default function TaxComparisonCalculator() {
  ...
  const chartData = [
    {
      name: cityA,
      Federal: taxA.federal,
      State: taxA.stateIncome,
      City: taxA.cityIncome,
      Property: taxA.propertyTax,
      FICA: taxA.fica,
      Sales: taxA.salesTax,
    },
    {
      name: cityB,
      Federal: taxB.federal,
      State: taxB.stateIncome,
      City: taxB.cityIncome,
      Property: taxB.propertyTax,
      FICA: taxB.fica,
      Sales: taxB.salesTax,
    },
  ];
  ...
  const biggestFactor = useMemo(() => {
    const categories: { label: string; diff: number }[] = [
      { label: "federal tax", diff: Math.abs(taxA.federal - taxB.federal) },
      { label: "FICA", diff: Math.abs(taxA.fica - taxB.fica) },
      {
        label: "state income tax",
        diff: Math.abs(taxA.stateIncome - taxB.stateIncome),
      },
      {
        label: "city income tax",
        diff: Math.abs(taxA.cityIncome - taxB.cityIncome),
      },
      {
        label: "property tax",
        diff: Math.abs(taxA.propertyTax - taxB.propertyTax),
      },
      { label: "sales tax", diff: Math.abs(taxA.salesTax - taxB.salesTax) },
      {
        label: "auto insurance",
        diff: Math.abs(taxA.autoInsurance - taxB.autoInsurance),
      },
      { label: "HOA fees", diff: Math.abs(hoaAnnualA - hoaAnnualB) },
    ];
    const top = categories.reduce((a, b) => (b.diff > a.diff ? b : a));
    return top.diff < 1 ? "overall taxes" : top.label;
  }, [taxA, taxB, hoaAnnualA, hoaAnnualB]);

  const youKeepA = taxA.takeHomePay - taxA.autoInsurance - hoaAnnualA;
  const youKeepB = taxB.takeHomePay - taxB.autoInsurance - hoaAnnualB;
  ...
  const handleCSVDownload = useCallback(() => {
    ...
    const dataRows = [
      ...
      [
        ...
        taxA.stateIncome,
        taxB.stateIncome,
        taxB.stateIncome - taxA.stateIncome,
      ],
      [
        "City Income Tax",
        taxA.cityIncome,
        taxB.cityIncome,
        taxB.cityIncome - taxA.cityIncome,
      ],
      [
        "Property Tax (est.)",
        taxA.propertyTax,
        taxB.propertyTax,
        taxB.propertyTax - taxA.propertyTax,
      ],
      [
        "Sales Tax (est.)",
        taxA.salesTax,
        taxB.salesTax,
        taxB.salesTax - taxA.salesTax,
      ],
      ...(hoaAnnualA || hoaAnnualB
        ? [["HOA Fees", hoaAnnualA, hoaAnnualB, hoaAnnualB - hoaAnnualA]]
        : []),
      [
        "Total Tax",
        taxA.totalTax,
        taxB.totalTax,
        taxB.totalTax - taxA.totalTax,
      ],
      [
        "Auto Insurance (est.)",
        taxA.autoInsurance,
        taxB.autoInsurance,
        taxB.autoInsurance - taxA.autoInsurance,
      ],
      [
        "Take-Home Pay (after tax)",
        taxA.takeHomePay,
        taxB.takeHomePay,
        taxB.takeHomePay - taxA.takeHomePay,
      ],
      [
        "You Keep (after tax, auto, HOA)",
        youKeepA,
        youKeepB,
        youKeepB - youKeepA,
      ],
  ...
  }, [cityA, cityB, taxA, taxB, hoaAnnualA, hoaAnnualB, youKeepA, youKeepB]);
  ...
  const rows: { label: string; a: number; b: number }[] = [
    ...
    ...(hoaAnnualA || hoaAnnualB
      ? [{ label: "HOA Fees", a: hoaAnnualA, b: hoaAnnualB }]
      : []),
  ];
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              ...
              <Input
                ...
                value={salaryInput}
                onChange={(e) => setSalaryInput(parseInt(e.target.value) || 0)}
                className="font-mono"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                ...
                max={10}
                value={deps}
                onChange={(e) => setDeps(parseInt(e.target.value) || 0)}
              />
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Median"
                    value={homeValueA}
                    onChange={(e) => setHomeValueA(e.target.value)}
                    className="text-xs"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    placeholder="0"
                    value={hoaA}
                    onChange={(e) => setHoaA(e.target.value)}
                    className="text-xs"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Median"
                    value={homeValueB}
                    onChange={(e) => setHomeValueB(e.target.value)}
                    className="text-xs"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    placeholder="0"
                    value={hoaB}
                    onChange={(e) => setHoaB(e.target.value)}
                    className="text-xs"
                  />
                </div>
              </div>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <div className="grid gap-4 sm:grid-cols-3 mb-6">
              ...
              <div
                className={`rounded-lg border p-3 text-center ${totalDiff > 0 ? "border-michigan-forest/30 bg-michigan-forest/5" : "border-michigan-coral/30 bg-michigan-coral/5"}`}
              >
                ...
                <p className="text-[10px] text-muted-foreground">
                  {totalDiff > 0 ? `${cityB} saves you` : `${cityA} saves you`}
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left">
                    <th className="pb-2 text-xs text-muted-foreground">
                      Category
                    </th>
                ...
                <tbody>
                  ...
                  <tr className="font-semibold">
                    ...
                    <td className="py-2 text-xs text-right">
                      {fmt(taxA.totalTax + hoaAnnualA)}
                    </td>
                    <td className="py-2 text-xs text-right">
                      {fmt(taxB.totalTax + hoaAnnualB)}
                    </td>
                    <td className="py-2 text-xs text-right">
                      <DiffCell
                        a={taxA.totalTax + hoaAnnualA}
                        b={taxB.totalTax + hoaAnnualB}
                      />
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="py-2 text-xs text-right text-muted-foreground">
                      {fmt(taxA.autoInsurance)}
                    </td>
                    <td className="py-2 text-xs text-right text-muted-foreground">
                      {fmt(taxB.autoInsurance)}
                    </td>
                    <td className="py-2 text-xs text-right">
                      <DiffCell a={taxA.autoInsurance} b={taxB.autoInsurance} />
                    </td>
                  </tr>
                  <tr className="font-bold text-primary">
                    <td className="py-2 text-xs">YOU KEEP</td>
                    <td className="py-2 text-xs text-right">{fmt(youKeepA)}</td>
                    <td className="py-2 text-xs text-right">{fmt(youKeepB)}</td>
                    <td className="py-2 text-xs text-right">
                      <DiffCell a={-youKeepA} b={-youKeepB} />
                    </td>
                  </tr>
                  <tr>
                    <td className="py-2 text-xs text-muted-foreground">
            ...
            <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 mt-4">
              <p className="text-xs text-foreground">
                <strong>Key insight:</strong> Living in{" "}
                {totalDiff > 0 ? cityB : cityA} on a {fmt(salary)} salary saves
                ~{fmt(Math.abs(totalDiff))}/year in taxes. The biggest factor:{" "}
                {biggestFactor}.
              </p>
            </div>
            ...
            <div className="flex items-start gap-2 mt-3 text-[9px] text-muted-foreground">
              ...
              <span>
                Estimated for educational purposes. Uses representative rates.
                Property tax based on median home value unless overridden. Auto
                insurance from 2025 rate filings. Consult a tax professional.
                Sources: MI Treasury, IRS, city ordinances, Quadrant Info
                Services.
              </span>
            </div>
          </CardContent>
