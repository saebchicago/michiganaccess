...
const MOMENTS = [
    ...
    body: (
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <Badge className="bg-amber-700 text-white text-[9px]">
    ...
    body: (
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 h-3 bg-muted rounded-full overflow-hidden flex">
            <div
              className="h-full bg-michigan-teal rounded-full"
              style={{ width: "39%" }}
            />
          </div>
  ...
  {
    title: "6 programs match these rules",
    body: (
      <div className="flex flex-wrap gap-1.5">
        ...
        <p className="w-full text-[10px] text-muted-foreground mt-1">
          3 questions, zero data stored
        </p>
      </div>
    ),
    cta: "See how eligibility rules work",
...
export default function LiveDemoPreview() {
  ...
  useEffect(() => {
    const timer = setInterval(
      () => setIdx((prev) => (prev + 1) % MOMENTS.length),
      4000,
    );
