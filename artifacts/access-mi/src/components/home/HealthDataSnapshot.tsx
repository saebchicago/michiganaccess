...
import {
  buildStateSnapshotMetrics,
  buildCountySnapshotMetrics,
...
function AnimatedKPIValue({ value }: { value: string }) {
  ...
  const { value: animated, ref } = useCountUp<HTMLSpanElement>(
    isNaN(num) ? 0 : num,
    1200,
  );
...
const KPI_DESCRIPTIONS: Record<string, string> = {
  "Insurance Rate":
    "Share of residents with health insurance coverage. Source: ACS and County Health Rankings.",
  "Life Expectancy":
    "Average expected lifespan based on current mortality patterns. Source: MDHHS Vital Records and CDC.",
  "Mental Health Access":
    "Access index summarizing available mental health care and telehealth reach. Source: CMS and state data.",
  "ER Visit Rate":
    "Emergency department visits per 100,000 residents, used as a strain indicator. Source: MDHHS hospital utilization data.",
};
...
export default function HealthDataSnapshot() {
    ...
    return [
      {
        ...
        delta: countyData
          ? getDelta(
              countyData.insuranceRate,
              MICHIGAN_AVERAGES.insuranceRate,
              true,
            )
          : null,
        stateDelta: "+2.8%",
      ...
      },
      {
        ...
        delta: countyData
          ? getDelta(
              countyData.lifeExpectancy,
              MICHIGAN_AVERAGES.lifeExpectancy,
              true,
            )
          : null,
        stateDelta: "−0.3",
      ...
      },
      {
        ...
        delta: countyData
          ? getDelta(
              countyData.mentalHealthAccess,
              MICHIGAN_AVERAGES.mentalHealthAccess,
              true,
            )
          : null,
      ...
      },
      {
        ...
        delta: countyData
          ? getDelta(
              countyData.erVisitRate,
              MICHIGAN_AVERAGES.erVisitRate,
              false,
            )
          : null,
  ...
  const snapshotMetrics = useMemo(() => {
    return county
      ? buildCountySnapshotMetrics(county)
      : buildStateSnapshotMetrics();
  }, [county]);

  const sectionTitle = county
    ? `${county} County Health at a Glance`
    : "Michigan Health at a Glance";

  return (
    <section
      id="data-snapshot"
      className="py-14 bg-slate-900 text-white"
    ...
    >
      <div className="container max-w-6xl">
        ...
        >
          <Badge
            variant="outline"
            className="mb-3 text-xs uppercase tracking-wider border-cyan-400/40 text-cyan-300"
          >
                  ...
                  {kpi.hasData ? (
                    <>
                      ...
                      {county && kpi.delta ? (
                        ...
                        <p
                          className={`text-[11px] mt-0.5 flex items-center gap-1 ${kpi.up ? "text-emerald-400" : "text-rose-400"}`}
                        >
                          {kpi.up ? (
                    ...
                    <>
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] border-slate-600 text-slate-500 mt-1"
                      >
                        Data pending
                      </Badge>
                    </>
            ...
            <Card className="bg-slate-800/60 border-slate-700/50">
              <CardContent className="pt-5 pb-3">
                ...
                <p className="text-[11px] text-slate-400 mb-3">
                  Prevalence rates (%) - Source: CDC BRFSS
                </p>
                ...
                <div className="flex gap-4 mt-2 justify-center">
                  <span className="text-[10px] text-slate-400 flex items-center gap-1">
                    <span
                      ...
                      style={{ background: CHART_COLORS.mi }}
                    />{" "}
                    Michigan
                  </span>
                  <span className="text-[10px] text-slate-400 flex items-center gap-1">
                    <span
                      className="h-2 w-2 rounded-full"
                      style={{ background: CHART_COLORS.us }}
                    />{" "}
                    National
                  </span>
                </div>
          ...
          >
            <Card className="bg-slate-800/60 border-slate-700/50">
              <CardContent className="pt-5 pb-3">
                ...
                <p className="text-[11px] text-slate-400 mb-3">
                  Insurance coverage & telehealth adoption - Source: CMS, MDHHS
                </p>
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={accessTrends}>
                    ...
                    <RechartsTooltip
                      formatter={(v: number, name: string) => [
                        `${v}%`,
                        name === "insured" ? "Insured" : "Telehealth",
                      ]}
                      contentStyle={{
                        fontSize: 12,
                        borderRadius: 8,
                    ...
                    <Area
                      ...
                      dataKey="insured"
                      name="Insured Rate (%)"
                      stroke={CHART_COLORS.insured}
                      fill={CHART_COLORS.insured}
                      fillOpacity={0.15}
                      strokeWidth={2}
                    />
                    <Area
                      ...
                      dataKey="telehealth"
                      name="Telehealth (%)"
                      stroke={CHART_COLORS.telehealth}
                      fill={CHART_COLORS.telehealth}
                      fillOpacity={0.1}
                      strokeWidth={2}
                    />
                  </AreaChart>
                ...
                <div className="flex gap-4 mt-2 justify-center">
                  <span className="text-[10px] text-slate-400 flex items-center gap-1">
                    <span
                      className="h-2 w-2 rounded-full"
                      style={{ background: CHART_COLORS.insured }}
                    />{" "}
                    Insured Rate
                  </span>
                  <span className="text-[10px] text-slate-400 flex items-center gap-1">
                    <span
                      className="h-2 w-2 rounded-full"
                      style={{ background: CHART_COLORS.telehealth }}
                    />{" "}
                    Telehealth
                  </span>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-300">
                    Black infants in Michigan face 3.3× the mortality rate of
                    white infants (MDHHS 2024) - among the widest gaps
                    nationally. Michigan's Maternal Health Equity Initiative
                    allocates $120M toward doula coverage and extended
                    postpartum Medicaid.
                  </p>
                ...
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  <Button
                    ...
                    className="gap-1.5"
                  >
                    <Link to="/equity">Equity Framework</Link>
                  </Button>
                </div>
              </div>
            </CardContent>
