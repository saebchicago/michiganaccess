...
export default function UninsuredSparkline({ county }: Props) {
  ...
  const r1 = getReliability(p1.value, p1.moe);
  const r2 = getReliability(p2.value, p2.moe);
  // A suppressed point makes the delta itself unreliable - don't show the
  // dumbbell/trend math if either endpoint is too noisy to trust.
  const deltaSuppressed = r1.status === "suppressed" || r2.status === "suppressed";
  ...
  return (
    <div className="space-y-2" data-trend-uninsured>
      <div className="flex items-center justify-between gap-2">
        ...
        {deltaSuppressed ? (
          <Badge variant="outline" className="text-[10px] px-1.5 py-0 gap-1 flex-shrink-0 border-red-300 text-red-600 dark:border-red-800 dark:text-red-400">
            Delta unreliable
          </Badge>
        ) : (
          <Badge
      ...
      {deltaSuppressed ? (
        <SuppressedEstimate source={u.source} />
      ) : (
        <>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col items-center min-w-[56px]">
              ...
              <div className="mt-0.5 flex items-center gap-1">
                <MoEBadge estimate={p1.value} moe={p1.moe} />
                {r1.status === "unavailable" && <MoeUnavailableNote />}
              </div>
            </div>
            ...
            <div className="flex flex-col items-center min-w-[56px]">
              ...
              <div className="mt-0.5 flex items-center gap-1">
                <MoEBadge estimate={p2.value} moe={p2.moe} />
                {r2.status === "unavailable" && <MoeUnavailableNote />}
              </div>
            </div>
          </div>
        </>
