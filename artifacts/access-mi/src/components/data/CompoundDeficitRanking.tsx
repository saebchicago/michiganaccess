...
export default function CompoundDeficitRanking() {
  ...
  const tierCounts = useMemo(() => {
    ...
    data.forEach((d) => {
      const t = d.tier as keyof typeof counts;
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            ...
            healthcare (20%) + SVI (15%) + EJ (10%) + energy (10%). Higher =
            more compounding barriers.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Tier summary */}
          <div className="grid grid-cols-4 gap-2 mb-4">
            {(["Critical", "High", "Moderate", "Low"] as const).map((tier) => (
              <div
                key={tier}
                className="text-center rounded-lg border border-border p-2"
              >
                <Badge className={`${TIER_COLORS[tier]} text-[10px] mb-1`}>
                  {tier}
                </Badge>
          ...
          <div className="space-y-1">
            {data.slice(0, 30).map((d, i) => (
              <div key={d.county}>
                <button
                  onClick={() =>
                    setExpanded(expanded === d.county ? null : d.county)
                  }
                ...
                >
                  ...
                  <span className="text-sm font-bold text-foreground tabular-nums w-12 text-right">
                    {d.compound.toFixed(1)}
                  </span>
                  <Badge
                    className={`${TIER_COLORS[d.tier]} text-[9px] w-16 justify-center`}
                  >
                    {d.tier}
                  </Badge>
                  <ChevronDown
                    className={`h-3 w-3 text-muted-foreground transition-transform ${expanded === d.county ? "rotate-180" : ""}`}
                  />
                </button>
                {expanded === d.county && (
                  <motion.div
          ...
          <p className="text-[10px] text-muted-foreground mt-4">
            Showing top 30. Compound index uses county-profile proxies until
            tract-level data is seeded. Source: Access Michigan Compound Index
            v1 methodology.{" "}
